...
다음 cycle 은 runner/profile scoped envelope comparison command 구현 self-review 와 Phase 4 다음 실행 후보 재평가를 수행한다.

- 범위: D125 spec/plan, `tests/Hps.Benchmarks/` envelope command 구현,
  `tests/Hps.Benchmarks.Tests/` envelope tests, root 상태 문서, 필요 시 review 문서.
- 검증: source/test/spec 대조, command smoke 결과 대조, `git diff --check`,
  필요 시 focused benchmark tests 및 solution build/test.
