...
- 날짜: 2026-07-11
- 구현 결과: 2026-07-14
- 상태: Implemented - depth 4 repeated gate accepted on 2026-07-14
- 관련 결정: D113, D114, D116, D117, D118, D240
- 목표: Windows RIO UDP에서 4096B 메시지 100 Hz open-loop 3회 모두 delivery hard gate를 통과한다.

## 1. 문제와 확인된 범위

변경 전 RIO UDP는 endpoint마다 receive slot 2개를 생성하고 `RIOReceiveEx`를 미리 post했다.
...
2026-07-14에 계획의 수락 경로를 실행했다.

### Red와 최소 Green

- RIO UDP preflight는 smoke 8/8, drop/payload error/pool rented 0이었고 기존 focused tests는 18/18이었다.
- blocked-handler burst test는 production 변경 전에 expected 5 / actual 3으로 실패했다.
- close-owner test도 단독 실행에서 expected 5 / actual 3으로 실패했다. 두 Red를 한 process에서 함께 실행했을 때는
  첫 실패 cleanup 영향으로 close-owner actual이 0이었으므로, 독립 실행으로 depth 2 한계를 다시 확인했다.
- production 변경은 `RioUdpEndpoint.ReceiveWindowSize` 2→4 한 줄뿐이다.
- 두 강화 test는 2/2, `RioTransportUdpTests`는 17/17, 전체 RIO tests는 56/56으로 통과했다.
- solution Release build는 경고 0/오류 0, solution tests는 520/520으로 통과했다.

### 반복 delivery gate

- load 3회와 open-loop 3회 모두 sent/received 3000/3000, drop 0, payload error 0, pool rented 0이었다.
- load actual rate는 99.8~100.0 Hz, p50은 175.0~180.4 us, p99는 1039.0~1424.9 us, UDP send HWM은 1이었다.
- open-loop actual rate는 99.9~100.0 Hz, p50은 176.2~192.4 us, p99는 1454.2~2131.8 us,
  UDP send HWM은 2~4였다.
- summary hard failure는 0이었다. load/open-loop p99 warning 2개는 기존 정책대로 report-only이며 수락 조건과 분리했다.
- raw report 6개는 고유 임시 경로에서만 검증했고 repository baseline으로 채택하지 않았다.

따라서 fixed depth 4 가설을 수락한다. RIO는 계속 explicit opt-in이고 default transport, IPv6,
UDP reliability와 latency hard gate 범위는 변경하지 않는다.
