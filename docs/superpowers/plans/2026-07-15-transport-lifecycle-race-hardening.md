...
- [ ] **Step 1: checkout과 작업 외 파일을 확인한다**
...
- [ ] **Step 1: `FakeTransport`에 TCP/UDP start 차단 seam을 추가한다**
...
- [ ] **Step 4: 두 테스트가 현재 production에서 assertion Red인지 확인한다**
...
Expected: 2 tests fail. 최소한 bind/listen release 전 Stop이 이미 완료됐다는 assertion failure가 보여야 한다. hang, timeout 또는 compile failure면 test seam부터 수정하고 production으로 넘어가지 않는다.
...
- [ ] **Step 1: 각 test class에 standalone connection reflection helper를 추가한다**
...
- [ ] **Step 2: RIO Stop 이후 registration Red를 추가한다**
...
- [ ] **Step 1: control-path lifecycle gate를 추가한다**
...
- [ ] **Step 2: `StartTcpAsync`를 gate wrapper와 기존 core로 분리한다**
...
- [ ] **Step 5: `Dispose`의 종료 표식을 Stop보다 먼저 게시한다**
...
- [ ] **Step 6: server 경합 Red와 전체 Server tests를 Green으로 만든다**
...
- [ ] **Step 1: RIO에 lock 내부 전용 stopped guard를 추가한다**
...
- [ ] **Step 2: RIO completion port의 생성/종료 전환을 같은 lock으로 보호한다**
...
- [ ] **Step 3: RIO UDP registration 실패 cleanup을 endpoint owner까지 확장한다**
...
- [ ] **Step 4: io_uring에 lock 내부 전용 stopped guard를 추가한다**
...
- [ ] **Step 5: native Red와 각 project 전체를 Green으로 만든다**
...
- [ ] **Step 1: Green 상태에서 중복과 범위를 점검한다**
...
- [ ] **Step 2: SAEA lifecycle 대조와 solution 전체를 검증한다**
...
- solution tests 전체 green. 기존 기준은 520/520이며 이번 신규 test 4개 때문에 예상 총수는 524이지만, skip/환경 조건을 포함한 실제 total을 결과 문서에 기록한다.

- [ ] **Step 3: Windows SAEA TCP/UDP 4096B x 100Hz target gate를 실행한다**
