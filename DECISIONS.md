...
- explicit runner baseline 은 현재 `local-win-x64-01/2026-06-24` 3-session 과
  `local-win-x64-01/2026-06-25` 2-session 을 가진다(D087).
  ...
  다만 두 번째 date root 는 아직 2-session 뿐이므로 D082 warning-as-failure 또는 CI latency gate 승격 조건에는 산입하지 않는다.
- 다음 Phase 4 단위는 `local-win-x64-01/2026-06-25/session-03` explicit runner baseline 수집으로 둔다.
