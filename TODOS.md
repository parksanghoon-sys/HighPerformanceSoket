...
- 현재 Codex가 자동으로 이어서 실행할 항목은 없다.
  - 최신 완료 단위: 2026-06-22 UDP optional lease sweep 구현 계획.
...
- [ ] `P2_LATER` UDP optional lease tracker 와 sweep 을 구현한다.
  ...
  - next step: 구현 계획 Task 1부터 진행한다. 순서는 (1) 내부 `UdpLeaseOptions`, (2) lease tracker activity, (3) 순수 sweep, (4) `BrokerUdpDatagramHandler` wiring 이다. host timer 는 그 다음 단위다.
