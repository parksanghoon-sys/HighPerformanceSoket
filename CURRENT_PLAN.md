...
- D169 기준 D168 passing artifact 는 fixed registration, zero-copy send, latency gate, default promotion 의 근거가 아니라
...
- D175 기준 D174 fix 이후 원격 `iouring-benchmark-artifacts.yml` run `28627435853`은 success 로 완료됐다.
  TCP baseline exit 134는 재발하지 않았고, TCP/UDP baseline/summary/history/envelope exit code 는 모두 0이다.
  TCP envelope 는 reference-summary-count 6, UDP envelope 는 reference-summary-count 9이며 둘 다 signal-count 0이다.
- D176 기준 D175 이후 후속 후보를 재평가했고, 다음 구현 단위는 fixed/zero-copy pump 연결이 아니라
  `IoUringRegisteredBufferSet` native register/unregister 를 Linux contract artifact 로 검증하는 것이다.
  설계는 `docs/superpowers/specs/2026-07-03-iouring-post-d175-next-scope-design.md`에 있다.
...
- D179 기준 D178 이후 다음 구현 단위는 TCP/UDP pump fixed-buffer 연결이나 zero-copy send 가 아니라
  fixed-buffer SQE submission contract evidence 다.
  설계는 `docs/superpowers/specs/2026-07-03-iouring-post-d178-next-scope-design.md`에 있다.
- D180 기준 D179 구현 계획을 작성했다.
  계획은 `docs/superpowers/plans/2026-07-03-iouring-fixed-buffer-submission-evidence.md`에 있고,
  fixed-write opcode shape 와 fixed-write helper/native completion evidence 의 2개 커밋 단위로 나뉜다.
- D181 기준 fixed-buffer SQE submission contract evidence local 구현을 완료했다.
  `OperationWriteFixed`, `TrySubmitWriteFixed`, helper shape test, Linux capability gated pipe 기반 native completion test 를 추가했다.
  local/Windows 에서는 capability guard 로 early-return 하고, 실제 Linux native completion 은 다음 원격 contract gate 에서 확인한다.
- D182 기준 fixed-buffer submission 흐름의 전체 사용 예제를 문서화했다.
  예제는 `docs/examples/iouring-fixed-buffer-submission-example.md`에 있고, internal backend contributor 용이다.
...
- D191 기준 WPF sample dashboard 를 실제 실행해 검토했고, TCP/UDP smoke 자체는 성공하지만
  TCP 카드와 UDP 카드가 같은 `LastSmokeSummary`를 공유하는 UI 의미 오류를 발견해 수정했다.
  `TcpSmokeSummary`와 `UdpSmokeSummary`를 분리하고 XAML binding 을 각각의 속성으로 바꿨다.
...
- D194 기준 사용자 push 이후 `iouring-linux-contract.yml` run `28833852810`을 실행했지만
  Linux restore 단계에서 `samples/Hps.Sample.Dashboard`의 Windows TFM 때문에 `NETSDK1100`으로 실패했다.
  원인은 D183~D192로 WPF sample dashboard가 solution에 포함된 뒤에도 Linux contract workflow가
  solution-level restore/build 를 계속 수행한 것이다. D194로 workflow restore/build 범위를
...
- D197 기준 D196 socket fixed-write evidence 를 로컬 구현했다.
  `IoUringFixedBufferSubmissionTests`에 test-only `LinuxSocketPair` helper 와
  `WriteFixed_WhenLinuxCapabilityAvailable_WritesRegisteredBufferSliceToSocketPair`를 추가했다.
  Windows/local 환경에서는 capability guard 로 native body 가 early-return 하며,
  focused fixed-buffer submission tests 3개와 `Hps.Transport.IoUring.Tests` 63개가 통과했다.
  다음 실행 지점은 사용자 push 이후 원격 `iouring-linux-contract.yml` artifact 에서
  socketpair test 가 capability `Available`, completion result 2, payload `{20,30}`으로 통과하는지 검토하는 것이다.
  이 remote gate 전까지는 TCP/UDP pump fixed-buffer 연결 근거로 쓰지 않는다.
- D198 기준 D197 socket fixed-write evidence 는 원격 `iouring-linux-contract.yml` run `28837405462`에서 통과했다.
  head SHA 는 `84af508110a1c104c8b484cf138e05c83f8893d8`이고,
  `Hps.Transport.IoUring.Tests` 63개가 executed/passed 63, failed 0으로 통과했다.
  TRX 기준 `WriteFixed_WhenLinuxCapabilityAvailable_WritesRegisteredBufferSliceToSocketPair`는 capability `Available`
  상태로 Passed 이고 stdout 에 `fixed socket write completion result: 2`를 남겼다.
  이 evidence 는 stream socket fd 에 fixed-write 가 동작한다는 계약을 닫지만,
  production TCP/UDP pump fixed-buffer 연결, zero-copy send, default promotion 의 직접 근거가 아니다.
  다음 실행 지점은 D198 evidence 기준으로 io_uring 후속 후보를 재평가하는 것이다.
- D199 기준 D198 이후 다음 단위는 production pump 변경이 아니라 TCP fixed-send lease owner 구현 계획으로 좁혔다.
  설계는 `docs/superpowers/specs/2026-07-07-iouring-post-d198-next-scope-design.md`에 있다.
  핵심 gap 은 `IoUringRegisteredBufferSet`의 fixed buffer registration lifetime 이
  `TransportConnection.InFlightSend`/`RefCountedBuffer` fan-out ownership lifetime 과 아직 결합되어 있지 않다는 점이다.
  다음 실행 지점은 `docs/superpowers/plans/2026-07-07-iouring-fixed-send-lease-owner.md` 구현 계획을 작성하는 것이다.
  TCP/UDP pump fixed-buffer 연결, zero-copy send, default promotion, latency hard gate 는 계속 제외한다.
- D200 기준 TCP fixed-send lease owner 구현 계획을 작성했다.
  계획은 `docs/superpowers/plans/2026-07-07-iouring-fixed-send-lease-owner.md`에 있고,
  pure lease ownership contract, queue-based real registration factory, Linux native lease evidence,
  remote contract gate documentation 의 4개 task 로 나뉜다.
  다음 실행 지점은 Task 1 pure lease ownership contract 를 Red-Green 으로 구현하는 것이다.
- D201 기준 D200 Task 1 pure lease ownership contract 를 구현했다.
  `IoUringFixedSendLease`와 `IIoUringFixedBufferRegistration`을 추가했고,
  lease dispose 가 registration owner 와 payload ref 를 정확히 1회 정리하는지 focused tests 로 검증했다.
  다음 실행 지점은 Task 2 queue-based real registration factory 다.
- D202 기준 D200 Task 2 queue-based real registration factory 를 구현했다.
...
- D204 기준 사용자 push 이후 원격 `iouring-linux-contract.yml` run `28840613527`을 실행했다.
  새 lease native evidence test 는 capability `Available` 상태로 통과했지만,
...
- D207 기준 D206 evidence 를 재평가했고, 다음 구현 후보는 TCP payload fixed-write integration 으로 정했다.
  단, production pump 가 기존 `InFlightSend` ref 와 fixed-write lease ref 를 섞어 double release 하지 않도록,
  send pump 전용 lease ref 획득/rollback 경계를 먼저 둔다.
  설계는 `docs/superpowers/specs/2026-07-08-iouring-post-d206-next-scope-design.md`에 있다.
  다음 실행 지점은 D207 구현 계획 작성이다.
- D208 기준 D207 설계를 TDD 구현 계획으로 쪼갰다.
...
- D210 기준 D208 Task 2 TCP payload fixed-write helper 를 완료했다.
  `IoUringTransport.SendInFlightAsync`는 length prefix 를 기존 `SendArrayAsync` path 로 유지하고,
  non-empty payload 만 `SendFixedPayloadAsync`에서 `TrySubmitWriteFixed`로 전송한다.
...
- D213 기준 D212 이후 다음 단위는 fixed-write production 재시도가 아니라
  `iouring-linux-contract.yml` hang diagnostics 보강으로 정했다.
  설계는 `docs/superpowers/specs/2026-07-08-iouring-contract-hang-diagnostics-design.md`에 있다.
  다음 실행 지점은 D213 구현 계획 작성이다.
- D214 기준 D213 구현 계획을 작성했다.
...
- D222 기준 opt-in fixed registered payload helper shape 를 추가했다.
  `IoUringTransport.SendFixedRegisteredPayloadAsync(...)`는 registry lookup 성공 시 `TrySubmitWriteFixed(...)`를 사용할 수 있는
...
- D225 기준 D224 이후 후속 후보를 재평가했다.
  ...
  D226 기준 이 후속 설계를 TCP-first registered payload pool 설계로 구체화했다.
  설계 문서는 `docs/superpowers/specs/2026-07-09-iouring-registered-payload-pool-design.md`에 있다.
  핵심 방향은 `RefCountedBuffer` 반환 owner 경계, `TcpFrameAssembler` source 경계,
  io_uring internal registered payload pool, explicit fallback, UDP 제외 범위다.
  다음 실행 지점은 사용자가 D226 설계를 검토하는 것이다.
- D227 기준 현재 구현된 Interface Server 를 실제로 사용하는 방법을 문서화했다.
...
native wrapper shape Task 3 capability probe wiring 도 완료했다.
...
native wrapper shape Task 5 state documents/full verification 도 완료했다(D135).
...
TCP-first io_uring queue/pump Task 1 native SQE/CQE/enter shape 도 완료했다.
`IoUringNative`에 TCP `SEND`/`RECV` opcode, `io_uring_enter` wrapper, SQE/CQE ABI struct 를 추가했다.
최신 focused 검증은 `IoUringSubmissionShapeTests` 1개와 `Hps.Transport.IoUring.Tests` 18개 통과다.
TCP-first io_uring queue/pump Task 2 operation registry and completion context 도 완료했다.
`IoUringOperationKind`, `IoUringCompletion`, `IoUringOperationContext`, `IoUringOperationRegistry`를 추가해
CQE `user_data` token 을 managed operation context 로 라우팅하는 경계를 만들었다.
context 는 `WaitAsync` 이후 completion 을 정확히 한 번만 허용하고, `Reset`으로 다음 operation 재사용 상태를 준비한다.
최신 focused 검증은 `IoUringOperationRegistryTests` 6개와 `Hps.Transport.IoUring.Tests` 24개 통과다.
최신 표준 검증은 solution build 경고 0/오류 0, solution tests 404개 통과, `git diff --check` 통과다.
TCP-first io_uring queue/pump Task 3 shared completion loop boundary 도 완료했다.
`IoUringCompletionLoop`는 CQE `user_data` token 을 registry context 로 dispatch 하는 단일 경계를 제공한다.
현재 단계에서는 native CQ drain thread 없이 pure dispatch 와 lifecycle shell 만 구현하고,
실제 TCP resource/listener 및 receive/send SQE 제출은 후속 Task 로 분리한다.
최신 focused 검증은 `IoUringCompletionLoopTests` 4개와 `Hps.Transport.IoUring.Tests` 28개 통과다.
최신 표준 검증은 solution build 경고 0/오류 0, solution tests 408개 통과, `git diff --check` 통과다.
TCP-first io_uring queue/pump Task 4 TCP resource and listener wiring 도 완료했다.
`IoUringConnectionListener`와 `IoUringTcpConnectionResource`를 추가했고,
`IoUringTransport`가 Linux capability available 상태에서 queue/operation registry/completion loop 를 준비한 뒤
TCP listen/connect/accept resource boundary 를 만들도록 연결했다. 실제 receive/send SQE 제출은 아직 시작하지 않는다.
`Hps.Transport.IoUring` backend assembly 가 `TransportConnection` runtime internal 을 사용할 수 있도록
`Hps.Transport` friend assembly 경계도 RIO와 같은 방식으로 정렬했다.
최신 focused 검증은 `IoUringTransportTcpTests` 4개와 `Hps.Transport.IoUring.Tests` 32개 통과다.
최신 표준 검증은 solution build 경고 0/오류 0, solution tests 412개 통과, `git diff --check` 통과다.
TCP-first io_uring queue/pump Task 5 TCP receive pump 도 구현했다.
`IoUringQueue.TrySubmitReceive(...)`와 `TryDequeueCompletion(...)`을 추가하고,
`IoUringCompletionLoop`가 queue completion을 drain 해 registry context 로 dispatch 하도록 확장했다.
`IoUringTransport`는 connection 생성 후 receive loop를 시작해 positive completion 을 receive handler 로 전달하고,
0/음수 completion 또는 handler 예외는 connection close notify 로 수렴한다.
현재 Windows 검증에서는 low-level receive pump shape 와 Linux-gated loopback early-return 까지만 확인했고,
...
TCP-first io_uring queue/pump Task 7 state documents and full verification 도 완료했다(D137).
TCP-first pump 구현 boundary 는 shared `IoUringQueue`/`IoUringCompletionLoop`, reusable operation context,
...
Linux io_uring contract gate Task 3 state documents and decision 도 완료했다.
D138은 active decision index 와 archive decision 에 기록되어 있고, Linux contract workflow 는 로컬 검증까지 완료됐다.
다만 `workflow_dispatch` 원격 실행 결과 artifact 는 아직 없으므로, 실제 Linux available host syscall loopback 검증은
계속 open 상태다.
2026-06-29 원격 확인에서는 `git fetch origin master` 이후에도 로컬 `master`가 `origin/master`보다 24 commits ahead 이며,
...
- D162 원격 검토 결과: run `28497147332`은 workflow success 로 완료됐다.
  TCP/UDP baseline, summary, history, envelope exit code 는 모두 0이고, TCP/UDP raw report count 는 각각 6이다.
