...
- explicit runner baseline 은 현재 `local-win-x64-01/2026-06-24` 3-session 과
  ...
  같은 runner 의 두 date root 가 각각 3-session reference 를 갖췄으므로, 다음 Phase 4 단위는
  D082 warning-as-failure 또는 CI latency gate 승격 후보를 재평가하는 문서/정책 단위로 둔다.
  gate 구현은 자동으로 진행하지 않고 false failure 비용, runner 재현성, CI 보존 정책을 먼저 닫는다.
