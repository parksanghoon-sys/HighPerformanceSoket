...
현재 권장 후보는 UDP broker v1 wire/control 범위를 결정하고, 포함한다면 `1 datagram = 1 message`
publish/subscribe 경로를 별도 TDD 단위로 설계하는 것이다. 그 전에는 stable endpoint id/reconnect binding 이
v1 요구인지, UDP command wire format 을 TCP text command 와 맞출지 먼저 좁게 확인한다.

## 이번 단위의 검증 경로
- `dotnet test tests\Hps.Broker.Tests\Hps.Broker.Tests.csproj --no-restore --filter "FullyQualifiedName~SubscriptionTable_Contract_ExposesBrokerSubscriberSnapshot|FullyQualifiedName~CopySubscribers_WhenBrokerSubscriberDestinationIsUsed"`
- `dotnet test tests\Hps.Broker.Tests\Hps.Broker.Tests.csproj --no-restore`
