...
- Linux io_uring workflow와 30초 반복·soak·native backend mixed workload evidence는 아직 없다.

## 다음 단일 작업 단위

### D243 Task 7 Linux io_uring mixed artifact workflow

- io_uring benchmark artifact workflow에 mixed 전용 root/date/session 경로를 추가한다.
- `--mixed-load-open-loop --backend iouring` 30초 실행을 3회 독립 report로 수집한다.
- 각 실행 실패를 누적한 `IOURING_MIXED_EXIT`를 기존 final gate에 포함하고 workflow summary에 별도로 기록한다.
- mixed report는 기존 TCP/UDP baseline summary/history/envelope 입력 root와 분리한다.
- production Broker/Protocol/Transport와 legacy baseline workflow 계약은 변경하지 않는다.
...
6. [다음] Linux io_uring mixed artifact workflow.
