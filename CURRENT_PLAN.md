...
현재 권장 후보는 Broker subscription value 를 TCP/UDP endpoint 중심 모델로 전환할지 설계/구현하는 것이다.
이 작업은 `SubscriptionTable`, `BrokerPublisher`, TCP command handler, 향후 UDP broker 결선에 영향을 주므로 별도 단위로 다룬다.
...
- Broker subscription value 의 endpoint 중심 전환
- 백프레셔 기본 정책 정합성 결정
- UDP broker v1 범위 결정
- protocol error 응답
