...
다음 cycle 은 TCP-first io_uring queue/pump implementation plan Task 3 shared completion loop boundary 를 TDD로 구현한다.

- 범위: `docs/superpowers/plans/2026-06-29-iouring-tcp-first-pump.md` Task 3,
  `IoUringCompletionLoop`, registry dispatch boundary, pure completion routing tests.
- 검증: reflection/behavior assertion Red, focused completion loop tests Green, io_uring test project,
...
- 현재 상태: io_uring source/test project, capability probe, opt-in transport root type 이 존재한다.
  ...
  operation registry/context 가 존재한다.
- 다음 산출물: CQE를 registry context 로 dispatch 하는 completion loop boundary.
...
- CI artifact 자동 채택, pull_request trigger, schedule trigger
- io_uring TCP resource/listener, receive/send pump 구현
- stable identity 인증/권한 검증, persistence, payload replay
