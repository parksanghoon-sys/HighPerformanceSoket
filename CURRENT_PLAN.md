...
다음 cycle 은 TCP-first io_uring queue/pump implementation plan Task 5 TCP receive pump 를 TDD로 구현한다.

- 범위: `docs/superpowers/plans/2026-06-29-iouring-tcp-first-pump.md` Task 5,
  `IoUringTransport`, `IoUringTcpConnectionResource`, `IoUringQueue` receive SQE submission/completion handling.
- 검증: Linux-available gated loopback Red/Green, Windows/unavailable host early-return 유지, io_uring test project,
...
- 현재 상태: io_uring source/test project, capability probe, opt-in transport root type 이 존재한다.
  ...
  operation registry/context, completion loop dispatch boundary, TCP listener/resource skeleton 이 존재한다.
- 다음 산출물: Linux available host 에서 TCP receive bytes 를 handler 로 전달하는 receive pump.
