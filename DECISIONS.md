...
- explicit runner baseline 은 현재 `local-win-x64-01/2026-06-24` 3-session 과
  `local-win-x64-01/2026-06-25/session-01`을 가진다(D086).
  runner root history 는 4-session, hard-passed true, warning-count 0, comparison-compatible true 다.
  현재 explicit runner envelope 는 load p99 max 921.1 us, open-loop p99 max 1077.4 us 이며,
  hard gate, warning, comparison mismatch 는 없다.
  다만 두 번째 date root 는 아직 1-session 뿐이므로 D082 warning-as-failure 또는 CI latency gate 승격 조건에는 산입하지 않는다.
- 다음 Phase 4 단위는 `local-win-x64-01/2026-06-25/session-02` explicit runner baseline 수집으로 둔다.
  CI/warning-as-failure 설계는 같은 runner 의 두 번째 date root 에도 3-session reference 를 쌓은 뒤 다시 평가한다.
