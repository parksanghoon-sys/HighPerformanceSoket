...
다음 작업은 이 보정된 설계를 기준으로 one-deep pre-post 구현 계획을 작성하는 것이다.

## 이번 단위의 검증 경로

이번 cycle 은 리뷰 반영된 RIO UDP receive window hardening 설계를 기준으로 구현 계획을 작성한다.
...
- 검증: 설계 리뷰 B1~B5 대조, close/resource owner 경계 재확인,
  no-prefetch vs one-deep pre-post vs bounded prefetch failure mode 대조, 구현 계획 placeholder scan,
  계획 작성 전 `git diff --check`.
