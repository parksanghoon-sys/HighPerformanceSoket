...
- D243 - 기존 baseline을 보존하고 data/control 분리 TCP connection을 사용하는 독립 mixed workload command/report로 새 운영 목표를 검증한다. rate는 `sent - 1` interval, latency는 worst subscriber로 판정하며 harness 자원 상한을 실행 전에 검증한다.
