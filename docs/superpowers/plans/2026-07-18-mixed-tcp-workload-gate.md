...
- [x] **Step 1: CLI contract assertion Red를 추가한다**
...
- [x] **Step 2: command-line 모델과 mixed parser를 최소 Green으로 구현한다**
...
- [x] **Step 4: focused, benchmark 전체와 legacy parser 회귀를 Green으로 확인한다**
...
- [x] **Step 5: 1초 CLI smoke로 report와 exit code를 확인하고 commit한다**
...
```bash
timeout --signal=TERM --kill-after=10s 1m dotnet run --project tests/Hps.Benchmarks/Hps.Benchmarks.csproj --no-build --no-restore -- \
  --mixed-load-open-loop \
  --backend iouring \
  --data-rate-hz 100 \
  --duration-seconds 30 \
...
2026-07-21 workflow run `29801941712`의 Linux solution restore 회귀를 project-scoped build로 교정한 뒤,
run `29802726026`이 pushed SHA `b7ffa22d80864d2c9e69fef1bac1dc6777efbfc1`에서 restore/build를 통과했다.
이 run은 TCP raw 2개 뒤 정지했고 mixed 3회 data p99가 `5668.4~6791.6us`로 실패했다.

raw failure에 따라 D244를 별도 TDD 단위로 열어 TCP pending operation explicit cancel, receive/send pump lifetime,
16KiB recv block과 per-command watchdog을 로컬 구현했다. local io_uring 97/97, benchmark 223/223,
solution 640/640과 Release build 경고 0/오류 0을 확인했다.

사용자가 D244를 push한 뒤 `iouring-linux-contract.yml`과 `.github/workflows/iouring-benchmark-artifacts.yml`을
같은 SHA에서 `workflow_dispatch`로 실행한다. 다음을 직접 확인한다.

- workflow checkout SHA가 검토한 local commit SHA와 같다.
- contract의 peer-open pending recv `StopAsync` test와 전체 native tests가 green이다.
