...
- 여러 frame 이 한 chunk 에 붙는 경우, 0 length frame, 적대적 chunk fuzz, 실제 command codec 연결은 아직 후속 단위로 남아 있다.
- D013 기준으로 이번 기능 단위 완료 후 다음 구현은 사용자 리뷰 뒤 진행한다.

## 다음 단일 작업 단위
사용자 리뷰 대기.

리뷰 후 계속 진행 지시가 있으면 TCP frame edge/fuzz coverage 를 보강할지, command codec 으로 넘어갈지 실제 코드와 `PLAN.md` 기준으로 다시 판단한다.
UDP receive backpressure 정책(Q1)은 fan-out/backpressure 결정과 맞물리므로 성급히 구현하지 않고 별도 설계 단위로 둔다.

## 이번 단위의 검증 경로
- `TcpFrameAssembler` 타입/API 부재를 Red 로 먼저 확인한다.
- fragmented header/payload 조립, maxPayload 초과 거부, partial payload dispose 반환을 직접 public API 테스트로 확인한다.
...
- 결과: Protocol 전체 통과 3. 전체 통과 47, 실패 0, 건너뜀 0. 빌드 경고 0, 오류 0.
...
- 여러 frame 이 같은 TCP chunk 에 들어오는 경계와 적대적 chunk fuzz
