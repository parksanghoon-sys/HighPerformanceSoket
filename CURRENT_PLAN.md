...
다음 cycle 은 runner/profile scoped envelope comparison command Task 4 writer/Program wiring 을 TDD로 구현한다.

- 범위: `tests/Hps.Benchmarks/BaselineEnvelopeComparisonWriter.cs`,
  `BaselineEnvelopeComparisonMarkdownWriter.cs`, `Program.cs`,
  `tests/Hps.Benchmarks.Tests/BaselineEnvelopeComparisonWriterTests.cs`,
  `tests/Hps.Benchmarks.Tests/BaselineEnvelopeProgramTests.cs`, root 상태 문서.
- 검증: writer JSON/Markdown Red assertion failure, Program CLI smoke Red failure,
  focused `BaselineEnvelope*` tests, optional local CLI smoke, `git diff --check`, solution build/test.
