...
- RIO UDP no-prefetch 는 D111 기준으로 pool ownership/backpressure 경계로 해석한다.
  handler 가 첫 datagram 을 처리하는 동안 다음 `ReceiveEx`를 post 하지 않기 때문에 내부 pool 대여는 누적되지 않는다.
  단, RIO provider 는 outstanding receive 가 없는 동안 도착한 datagram 을 나중에 보장 전달하지 않을 수 있으므로,
...
- explicit runner baseline 은 현재 `local-win-x64-01/2026-06-24` 3-session 과
  `local-win-x64-01/2026-06-25` 3-session 을 가진다(D088).
  ...
  같은 runner 의 두 date root 가 각각 3-session reference 를 갖췄지만, D082가 요구한 서로 다른 date root 3개 이상과
  별도 warning threshold 검토는 아직 충족하지 못했다(D089). 따라서 warning-as-failure 와 CI latency hard gate 는
  계속 보류한다. 다음 Phase 4 단위는 CI runner identity, artifact 저장 위치, local/CI baseline 분리,
  exit code 정책을 먼저 닫는 CI artifact-only benchmark 정책 설계로 둔다.
