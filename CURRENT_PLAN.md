...
- 다음 실행 지점: D199 설계에 따라 TCP fixed-send lease owner 구현 계획을 작성한다.
  계획은 `IoUringFixedSendLease` 또는 동등한 internal owner 의 contract tests, idempotent cleanup,
  registered buffer lifetime, payload ref 반환 경계를 Red-Green 단위로 쪼갠다.
  production TCP pump fixed-write 연결은 계획/계약 검증 이후 별도 후속으로 둔다.
