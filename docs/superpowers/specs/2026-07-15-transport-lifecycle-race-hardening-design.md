...
- 목표: `BrokerServer`와 SAEA/RIO/io_uring transport에서 시작과 종료가 겹쳐도 종료 뒤 새 listener, connection, UDP endpoint 또는 pump가 살아남지 않게 한다.
...
SAEA의 `Register*`는 `_gate` 안에서 `EnsureRunningLocked()`를 호출하므로 Stop 뒤 목록 등록은 이미 막고 있다.
하지만 review follow-up에서 세 backend 모두 목록 등록 뒤 pump 시작이 별도 문장이라 Stop snapshot 뒤 late pump가 생길 수 있음을 확인했다.
최종 변경은 server orchestration, 등록 guard와 등록-pump 시작 원자성을 함께 적용한다.

## 2. 대안 비교

### A. Server lifecycle 직렬화 + transport 등록·pump 시작 직렬화 - 채택
...
- SAEA/RIO/io_uring connection과 UDP endpoint는 같은 lock 안에서 등록과 pump 생성을 끝낸다.
...
`BrokerServer` 경로는 안전해지지만 직접 SAEA/RIO/io_uring을 사용하는 sample, benchmark, library consumer는
`ListenTcpAsync`/`BindUdpAsync`/`ConnectTcpAsync`와 `StopAsync` 경합에 계속 노출된다.

### C. Transport registration guard만 추가 - 제외
...
- connection과 UDP endpoint는 같은 `_gate` 안에서 목록 등록과 receive/send pump 시작까지 완료한다.
...
- connection과 UDP endpoint 등록, pump task 생성과 connection send-task 추적은 Stop snapshot 전 같은 `_gate` 안에서 끝낸다.
...
최초 구현은 SAEA의 `EnsureRunningLocked()` 등록 계약을 그대로 유지했다. 그러나 review follow-up에서
SAEA도 connection/UDP 등록과 pump task 생성이 분리돼 direct transport Stop 뒤 late pump가 시작될 수 있음을 확인했다.
따라서 SAEA도 기존 `_gate` 안에서 실행 상태 검사, 목록 등록과 pump task 생성을 한 operation으로 완료한다.
...
3. connection/UDP 등록과 pump 생성·추적 사이에는 Stop snapshot이 끼어들 수 없다.
4. 등록 또는 pump 시작이 거부된 resource는 caller local owner가 정확히 한 번 정리한다.
5. 종료 뒤 `LocalEndPoint`와 `UdpLocalEndPoint`는 `null`이며 listener/endpoint는 닫혀 있다.
6. transport stop은 한 server lifetime에서 실제 시작된 transport에 대해서만 수행한다.
7. receive/send/publish hot path에는 새 lock, semaphore, allocation을 추가하지 않는다.
8. transport의 one-shot stop/restart 의미와 default backend 선택은 바꾸지 않는다.
...
### Review follow-up Red 6/7/8: transport 등록과 pump 시작 사이 Stop 직렬화

SAEA/RIO/io_uring의 private connection registration에 pump-start 동작을 전달하고 그 동작을 명시적 signal로 차단한다.
차단 중 별도 task에서 Stop을 호출했을 때 Stop은 완료되지 않아야 하며, release 뒤 registration과 Stop이 순서대로 끝나야 한다.

최초 구현에는 등록과 pump 시작을 한 operation으로 표현하는 seam이 없어 세 backend 모두
`Assert.NotNull` expected non-null/actual null로 실패한다. 이 Red는 capability probe와 native syscall 없이 실행한다.
...
- SAEA/RIO/io_uring connection/UDP 등록과 pump 생성·추적을 같은 transport lock 안의 단일 operation으로 묶는다.
...
- 신규 SAEA registration-pump test와 `Hps.Transport.Tests` 전체.
- solution Release build와 전체 tests.

이 변경은 데이터 경로를 바꾸지 않으므로 30초 성능 benchmark는 구현 수락의 필수 gate가 아니다.
다만 build/test 뒤 기존 4096B x 100 Hz Windows SAEA/RIO load/open-loop를 TCP/UDP 각각 한 번 실행해
...
- 구현 검토에서 registration과 pump 시작 사이의 Stop 경합을 발견해 follow-up Red 3개를 추가했다.
- 세 backend의 `RegisterAndStartPumps`로 connection/UDP 목록 등록과 pump 생성, io_uring send-task 추적을 원자화했다.
- follow-up 결과는 SAEA 45/45, RIO 58/58, io_uring 90/90, solution 528/528와 Release build 경고 0/오류 0이다.
- SAEA/RIO TCP/UDP 4096B x 100 Hz load/open-loop 여덟 run도 모두 3000/3000이며 drop, payload error, pool rented는 0이다.
...
- 최초 다섯 Red와 review follow-up Red 세 개는 각 production 변경 전에 예상 assertion failure로 확인한다.
