...
- hard gate 는 기존 summary 의 `hard-passed` 값을 session 단위로 AND aggregate 한다.
...
- `BenchmarkCommand` enum 값은 `SummarizeBaselineHistory`로 고정한다. `HistoryBaseline`은 command 의미가 덜 분명하므로 사용하지 않는다.

## History input 규칙

입력 root 는 `docs/benchmarks/baselines` 또는 특정 날짜 directory 둘 다 허용한다.

reader 는 입력 root 모양에 따라 아래처럼 bounded discovery 를 수행한다.

- 입력 root 자체의 directory 이름이 `YYYY-MM-DD`이면, 그 directory 하나만 날짜 root 로 본다.
  - `<root>/summary.json`: 과거 구현 흐름 때문에 날짜 root 가 `session-01(root)` 역할을 하는 경우.
  - `<root>/session-NN/summary.json`: 권장 session directory 구조.
- 입력 root 가 날짜 root 가 아니면, 바로 아래 child directory 중 이름이 `YYYY-MM-DD`인 directory 만 날짜 root 로 본다.
  - `<YYYY-MM-DD>/summary.json`
  - `<YYYY-MM-DD>/session-NN/summary.json`
...
- `failed-session-count`: `hard-passed == false`인 session 수
- `warning-count`
- `sessions`
...
load 또는 open-loop summary 가 없으면 해당 p99 값은 `null`로 둔다. `0`은 정상적으로 매우 낮은 latency 로 오해될 수 있으므로
부분 artifact 결함은 JSON `null`과 Markdown `-`로 드러낸다. 현재 schema 는 양쪽 runner 를 모두 기대하지만,
...
- `BenchmarkCommand.SummarizeBaselineHistory` 값을 추가한다.
...
- Ambiguity check: parent baseline root 와 날짜 root 입력, root summary 예외, session discovery depth, exit code, output key 를 명시했다.
