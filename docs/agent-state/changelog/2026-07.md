...
## 2026-07-21 (Codex - D244 io_uring TCP 종료·mixed 지연 보강)

### 원격 evidence

- run `29802726026`은 pushed SHA `b7ffa22d80864d2c9e69fef1bac1dc6777efbfc1`을 checkout했고 benchmark project restore/build를 통과했다.
- TCP load/open-loop 1회는 3000/3000과 pool/drop 0으로 report를 남겼지만 두 번째 load는 9분 이상 정지해 run을 취소했다.
- UDP load/open-loop 3회씩 6개는 모두 3000/3000과 hard correctness gate를 통과했다.
- mixed 3회는 data/control 각각 3000/3000, 100Hz, drop/end pending/pool/timeout/sequence/payload error 0이었다.
- mixed control p99는 `3386.3~4504.0us`로 통과했지만 data p99는 `5668.4~6791.6us`로 5ms를 모두 초과했다.
- artifact `8484559839`의 TCP raw 2개, UDP raw 6개, mixed raw 3개와 summary/history/envelope 격리를 직접 확인했다.

### TDD와 구현

- pump reference cleanup, receive pump Stop 관측, 16KiB recv block assertion Red를 각각 확인했다.
- queue async cancel helper, cancel control token routing, resource Dispose cancel call assertion Red를 확인했다.
- workflow command watchdog assertion Red를 확인한 뒤 TCP/UDP suite 4분과 mixed run 1분 상한을 추가했다.
- `IoUringTcpConnectionResource`는 receive/send pump reference가 모두 반환된 뒤 context, receive/length block과 fixed-send owner를 정리한다.
- `IoUringTransport`는 receive/send pump task를 모두 snapshot/drain한 뒤 completion loop와 queue를 dispose한다.
- queue는 `IORING_OP_ASYNC_CANCEL`을 target token으로 제출하고 completion loop는 token 0 cancel control CQE를 일반 operation과 분리한다.
- TCP recv block을 4KiB에서 16KiB로 늘려 10,240B payload와 현재 command envelope를 한 recv에 담는다.
- Linux available test에 peer-open pending recv 상태의 `StopAsync` 5초 종료 계약을 추가했다.

### 검증과 잔여 조건

- focused Red/Green 뒤 io_uring tests 97/97와 benchmark tests 223/223가 통과했다.
- Release 단일-node build는 경고 0/오류 0, solution tests는 640/640이다.
- `git diff --check`는 통과했다. 전체 format verify는 기존 파일 전반의 baseline whitespace 진단 때문에 실패해 이번 범위에서 자동 수정하지 않았다.
- native async cancel과 16KiB latency 효과는 Windows에서 검증할 수 없으므로 D244 push 후 Linux contract와 benchmark workflow를 같은 SHA에서 다시 실행해야 한다.

## 2026-07-21 (Codex - io_uring benchmark Linux restore 범위 교정)

### 원격 실패와 원인

- pushed SHA `75d81f54edea3930cf0fbffe266c2709acec07a6`로 workflow run `29801941712`를 실행했고 checkout SHA가 일치했다.
- `dotnet restore HighPerformanceSocket.slnx`가 Windows WPF sample에서 `NETSDK1100`으로 실패해 Build와 TCP/UDP/mixed benchmark가 실행되지 않았다.
- 실패 artifact는 summary 한 개뿐이며 모든 benchmark exit가 non-zero 또는 not-run이므로 성능 evidence로 수락하지 않았다.

### TDD 교정과 검증

- benchmark project 단위 restore/build 요구 테스트가 기존 YAML에서 실패하는 Red를 확인했다.
- workflow restore/build 대상을 `tests/Hps.Benchmarks/Hps.Benchmarks.csproj`로 제한하고 `EnableWindowsTargeting` 우회는 추가하지 않았다.
- benchmark build 경고 0/오류 0, benchmark tests 222/222와 solution tests 632/632가 통과했다.
- 교정 commit push와 동일 workflow 재실행 전까지 D243 Linux io_uring 수락은 열어 둔다.

## 2026-07-21 (Codex - D243 Windows mixed workload 수락 evidence)
...
- RIO 30초 3회는 실제 `RioTransport`로 각 stream 3000/3000, 100.0Hz, 최악 p99 1864.4us, p999 4052.3us, HWM 최대 3으로 모두 pass했다.
- D119 기본 backend인 SAEA 1,800초 soak는 각 stream 180000/180000, 100.0Hz, 최악 p99 1434.9us, p999 3105.8us, HWM 6으로 pass했다.
- 모든 report에서 drop/end pending/pool rented/timeout과 sequence/payload error가 0이었다.
- SAEA 반복, RIO 반복과 soak raw report 7개를 schema v2, backend, count/min/max, rate/latency hard gate로 재검증해 모두 `passed=true`를 확인했다.

### 범위와 잔여 조건

- raw artifact는 `artifacts/benchmarks/mixed/` 아래에 생성했고 `.gitignore` 규칙을 확인했다.
- 운영 data rate/subscriber 환경 변수가 없어 100Hz/N=1만 수락했으며 N=2는 기능 통합 테스트 근거만 보유한다.
- `origin/master`가 evidence HEAD보다 14커밋 뒤이므로 pushed-SHA Linux io_uring gate는 실행하지 않고 `P1_SOON`으로 남겼다.
- production Broker/Protocol/Transport는 변경하지 않았다.

## 2026-07-21 (Codex - D243 Linux io_uring mixed artifact workflow)

### TDD와 구현

- mixed root 부재 assertion Red를 확인한 뒤 runner/date/session 전용 artifact 경로를 추가했다.
- io_uring 100Hz, 30초, subscriber 1 mixed command를 3회 실행해 독립 schema v2 raw report를 수집한다.
- 한 실행이 실패해도 나머지를 계속 실행하고 누적 `IOURING_MIXED_EXIT`를 summary와 final failure gate에 연결했다.
- mixed root는 기존 TCP/UDP baseline summary/history/envelope 입력에서 분리했다.

### 검증과 범위

- workflow focused tests 9/9, benchmark tests 221/221과 `git diff --check`를 통과했다.
- 독립 리뷰의 Minor 테스트 공백을 정확한 전체 command/report path, 실패 누적과 지속 수집 assertion으로 보강했다.
- production Broker/Protocol/Transport와 legacy benchmark command는 변경하지 않았다.

## 2026-07-21 (Codex - D243 mixed workload CLI와 Program 연결)

### TDD와 구현

- command/property reflection assertion과 parser/help behavior assertion Red를 확인한 뒤 mixed 전용 command line을 추가했다.
- parser는 backend/data rate/duration/subscribers/report만 받고 protocol은 값 유무와 관계없이 전용 usage error로 거부한다.
- `MixedWorkloadOptions` 생성으로 입력 범위, count overflow와 128MiB latency 저장 상한을 socket과 배열 생성 전에 검증한다.
- Program은 mixed runner, schema v2 writer와 exit 0/1/2 계약을 연결하고 exact help line을 제공한다.

### 검증과 범위

- parser/Program focused tests 50/50, benchmark tests 220/220과 Release build 경고 0/오류 0을 통과했다.
- SAEA 1초 CLI smoke에서 data/control 100/100, drop/pending/pool/timeout 0과 schema v2 report를 확인했다.
- production Broker/Protocol/Transport, legacy baseline command와 workflow는 변경하지 않았다.

## 2026-07-21 (Codex - D243 N명 mixed TCP fan-out 구현)

### TDD와 구현

- subscriber 2명 integration과 두 latency summary 집계가 각각 기존 `NotSupportedException`으로 실패하는 assertion Red를 확인했다.
- data/control별 socket, pinned buffer, subscriber state와 receive task를 실행 전 고정 길이 배열로 만들고 logical index에 소유권을 고정했다.
- topic별 subscriber readiness가 계획 수에 도달한 뒤 모든 receive task를 시작하고 두 publisher는 기존 공통 absolute start를 재사용한다.
- 수신 합, subscriber별 최소/최대 수신 수, delivery 실패 subscriber 수와 sequence/payload error 합을 stream result로 집계한다.
- latency 원본 배열은 subscriber별로 유지하고 scratch 하나를 순차 재사용해 percentile과 전후반 growth를 독립 계산한 뒤 지표별 최댓값과 실패 수 합만 보존한다.
- receiver는 계획 수에서 멈추지 않고 server EOF/reset까지 읽어 terminal duplicate를 count하며, 초과 frame latency는 고정 배열에 쓰지 않고 delivery 실패 결과로 수렴한다.
- endpoint pending 0만 기다리고 server를 닫으면 client가 마지막 frame을 읽기 전 reset되어 99/100이 되는 실패를 재현했고, 모든 subscriber의 계획 수 도달 신호를 먼저 기다리도록 순서를 수정했다.
- setup 도중 일부 자원만 생성되거나 timeout/예외가 발생해도 배열을 순회해 socket 종료, task 관측과 pinned block 반환을 완료한다.

### 검증과 범위

- focused runner tests 14/14, benchmark tests 203/203과 solution tests 613/613이 통과했고 Release build는 경고 0/오류 0이다.
- subscriber 1명/2명과 duplicate receiver test를 묶어 20회 반복했고 모두 exact delivery, drop/pending/pool/timeout zero를 확인했다.
- public/internal runner signature, production Broker/Protocol/Transport와 CLI/workflow는 변경하지 않았다.

## 2026-07-21 (Codex - D243 단일 subscriber mixed TCP runner 구현)

### TDD와 구현

- runner type/`RunAsync`, subscriber latency seam, timestamp validation과 absolute deadline waiter 부재를 assertion Red로 확인했다.
- 최소 shell 뒤 SAEA 1초 integration이 `NotImplementedException`으로 실패하는 것을 확인하고 data/control 4-connection topology를 구현했다.
- publisher frame과 subscriber receive block은 주입한 16KiB pinned pool에서 connection별 한 번만 대여해 재사용한다.
- publisher는 공통 start tick의 `start + index * frequency / rate` target을 따르고, publisher별 reusable `AbsoluteDeadlineWaiter`가 같은 timer/value-task source를 절대 deadline으로 재무장한다.
- publisher/subscriber 장수명 async task 안에서 partial send/receive를 직접 처리해 per-message 보조 async state machine을 만들지 않는다.
- sequence, marker, pattern과 0/미래 timestamp를 검증하며 유효 latency만 subscriber 원본 배열에 기록한다.
- percentile scratch 하나로 p50/p99/p999와 전후반 p99를 계산하고 단일 subscriber summary를 stream result에 연결한다.
- 전체 timeout은 partial count result로 수렴하고 cleanup은 모든 시작 task를 관측한 뒤 socket, server와 client block을 정리한다.

### 독립 리뷰와 보완

- 첫 리뷰의 timestamp 무결성 Major는 `TryCalculateLatency` Red/Green과 payload error 반영으로 해소했다.
- per-message `Task.Delay`와 send/receive helper state machine Major는 reusable waiter와 장수명 task 내부 partial-I/O로 해소했다.
- publisher별 `PeriodicTimer` 생성 위상 Major는 공통 absolute target으로 timer를 재무장하는 waiter로 대체했다.
- 마지막 canceled/disposed fast-path Minor는 gate 안에서 상태를 먼저 판정하도록 순서를 바꿨다.
- 재검토에서 timer/cancellation/dispose 이중 완료나 use-after-dispose는 발견되지 않았다.

### 검증과 범위

- focused runner tests 12/12, benchmark tests 201/201가 통과했다.
- SAEA 1초 exact delivery integration은 별도 재실행 5회 모두 통과했다.
- Release 단일-node build는 경고 0/오류 0이고 solution tests 611/611이 통과했다.
- Task 4는 `SubscriberCount == 1`만 허용하며 Task 5 fan-out, CLI, workflow와 production 계층은 변경하지 않았다.

## 2026-07-21 (Codex - D243 mixed workload result/report hard gate 구현)

### TDD와 구현

- stream/run result, mixed writer와 mixed identity method 부재를 reflection assertion Red로 확인하고 각 shape를 먼저 Green으로 만들었다.
- behavior Red 뒤 subscriber별 exact delivery, `(sent - 1)` actual rate, worst-subscriber p99/p999와 global drop/pending/pool/timeout gate를 구현했다.
- `report-kind: mixed-tcp-workload`, `schema-version: 2`, canonical top-level/stream key와 legacy reader count 0을 고정했다.
- SAEA/RIO/io_uring은 기존 backend/environment metadata에 mixed 전용 benchmark profile을 결합한다.

### 검토와 검증

- 독립 리뷰는 구현 결함 없이 모든 count parameter의 음수 검증 테스트 확대만 권고했다.
- stream 14개와 run 9개 parameter theory를 추가해 권고를 해소했다.
- focused 56/56, benchmark 189/189, solution 599/599와 targeted format 검증이 통과했다.
- 기본 병렬 build worker 종료와 VSTest 시작 timeout이 각각 한 번 있었으나 같은 소스의 단일-node Release build는 경고 0/오류 0이고 전체 test 재실행도 통과했다.
- runner, CLI, socket과 production 계층은 변경하지 않았으며 Task 3 review stop에서 종료한다.

## 2026-07-20 (Codex - D243 mixed workload options 사전 검증 구현)

### TDD 증거

- reflection 계약 test는 production type 부재로 `Assert.NotNull(type)` 1개가 실패한 뒤 최소 shell에서 1/1 Green이 됐다.
- 기본값, 사용자 입력, 최소 경계, subscriber 상한, Int32 계획 수와 latency 저장 상한 behavior는 shell에서 10개 실패/1개 통과를 확인했다.
- 독립 리뷰 뒤 모든 고정 상수, 실제 Int64 overflow 변환과 정확한 128MiB 허용/거부 경계 테스트를 보강했다.
- 최소 검증 계산 구현 뒤 options 15/15, benchmark 133/133, solution 543/543가 통과했고 Release build는 경고 0/오류 0이다.

### 구현 범위

- `MixedWorkloadOptions`는 고정 payload/rate/gate 상수와 data/control message·delivery 수 및 `2 + 2N` client connection 수를 보관한다.
- latency 저장소는 모든 subscriber의 data/control 원본 `long[]`과 최대 stream 길이 scratch 하나의 payload를 합산한다.
- 입력과 `checked long` 산술 결과를 검증한 뒤 readonly property에 저장하며 socket, payload buffer 또는 latency 배열은 생성하지 않는다.
- result/report, runner, CLI와 10.24 Mbps 실행 evidence는 다음 Task로 남기고 Task 2 review stop에서 종료한다.

## 2026-07-20 (Codex - mixed TCP workload 설계/계획 검토 보완)

### 검토 결과

- D243 구현은 아직 시작되지 않았고 현재 executable은 legacy 4096B x 100Hz 단일 stream만 실행한다.
- Release build 경고 0/오류 0, solution tests 528/528를 확인했다.
- SAEA TCP legacy open-loop는 3000/3000, actual 99.8Hz, p99 623.9us, HWM 5, drop/payload error/pool rented 0이다.
- RIO TCP legacy smoke는 8/8, drop/payload error/pool rented 0이다.

### 설계/계획 보완

- fan-out latency hard gate는 모든 subscriber sample aggregate가 아니라 subscriber별 percentile의 최댓값을 사용한다.
- delivery failed subscriber count와 latency failed subscriber count를 분리한다.
- publisher actual rate는 첫/마지막 send completion 사이 `sent - 1` interval로 계산한다.
- options/CLI는 subscriber 256명과 latency 원본 배열 전체 및 재사용 scratch 하나의 payload 합계 128MiB를 실행 전에 검증한다.
- mixed raw report는 `report-kind: mixed-tcp-workload`, `schema-version: 2`를 기록한다.
- production code/tests는 변경하지 않았고 Task 2 options TDD 전 사용자 review stop을 유지한다.

## 2026-07-18 (Codex - mixed TCP workload implementation plan)

### 설계 승인과 코드 대조

- 사용자의 진행 승인으로 D243 written spec을 Accepted로 전환했다.
- benchmark parser/command line/Program, TCP runner, result/writer, legacy reader, transport diagnostics와 Linux workflow를 대조했다.
- legacy reader가 schema-version 1을 run report로 읽는 현재 계약 때문에 mixed raw report는 report kind와 schema-version 2로 분리한다.

### 구현 단위

- options와 checked count, stream/global result와 writer, 단일 subscriber runner, fan-out, CLI, workflow, evidence를 분리했다.
- 각 구현 단위는 assertion Red, 최소 Green, focused/benchmark test, 상태 문서, 단일 commit과 사용자 review stop을 가진다.
- connection별 publisher/subscriber buffer는 16KiB pinned pool에서 한 번 대여해 재사용한다.
- production Broker/Protocol/Transport와 legacy baseline은 mixed raw failure 근거가 생기기 전에는 수정하지 않는다.

### 상태

- implementation plan은 `docs/superpowers/plans/2026-07-18-mixed-tcp-workload-gate.md`다.
- 이번 cycle은 문서 전용이며 code/tests와 성능 evidence는 아직 없다.
- implementation plan 승인 뒤 첫 구현 단위는 `MixedWorkloadOptions` TDD다.

## 2026-07-18 (Codex - mixed TCP workload gate 설계)

### 목표 재정의

- legacy 4096B x 100 Hz는 backend 회귀 비교 기준선으로 유지한다.
- 새 운영 목표는 data 10,240B x 100 Hz 이상과 control 2,560B x 100 Hz를 동시에 처리하는 것이다.
- 기본 합계 payload rate는 10.24 Mbps이며 fan-out 송신량은 논리 구독자 수에 비례한다.

### 설계

- 기존 baseline 교체, 기존 single-stream runner 분기 확장, 독립 mixed command/report 세 접근을 비교했다.
- D243으로 TCP 전용 `--mixed-load-open-loop`와 전용 result/raw report를 채택했다.
- 논리 구독자별 data/control connection 분리, 공통 start signal, 독립 absolute schedule과 reusable client buffer를 정의했다.
- CLI는 backend 외에 data rate, duration, subscriber count만 조정한다.
- 기존 baseline summary/history/envelope와 UDP는 변경하지 않는다.

### 수락 경계

- subscriber별 exact delivery/order/payload, target rate 99% 이상, p99 5ms, p999 10ms를 stream hard gate로 둔다.
- 전체 gate는 drop 0, end pending 0, 종료 후 pool rented 0을 요구한다.
- Windows SAEA/RIO 30초 3회, 배포 우선 backend 1,800초 soak와 push된 SHA의 Linux io_uring 증거를 계획했다.
- 실제 최대 발행률, subscriber 수와 제어 데이터의 장애 시 전달 의미는 운영 입력 없이는 완료 주장 범위를 제한한다.

### 상태

- D242 RIO UDP SOCKADDR 임시 배열 제거는 `P2_LATER`로 이동했다.
- written spec은 `docs/superpowers/specs/2026-07-18-mixed-tcp-workload-gate-design.md`다.
- production code와 tests는 변경하지 않았고 사용자 review stop에서 종료한다.

## 2026-07-15 (Codex - D241 review stop 종료와 RIO UDP allocation 범위 재평가)

### review stop

- 사용자의 다음 진행 승인으로 D241 transport 등록-pump 시작 원자성 보강 결과를 확정했다.
- implementation follow-up의 deterministic tests, 전체 528/528, Release build와 SAEA/RIO target gate evidence를 유지한다.
- production code와 tests는 변경하지 않았다.

### 과거 finding 재대조

- `.claude/review/2026-06-26-rio-udp-receive-loop-review.md`를 현재 RIO UDP bounded receive window와 tests에 대조했다.
- M2의 handler exception close notify와 pool leak 검증은 현재 handler 예외·close/drain tests에 존재한다.
- M3의 teardown 경계는 posted slot을 보유한 상태의 close와 handler 예외 drain에서 `ReceivePool.RentedCount == 0`으로 검증된다.
- M1의 per-datagram payload registration은 미구현 누락이 아니라, handler fan-out 전 receive registration을 해제하는
  D113 소유권 결정과 slot별 payload owner 때문에 현재 유지한다.

### 다음 범위

- `DecodeSockaddrInet`의 `byte[4]`는 net9.0 `IPAddress(ReadOnlySpan<byte>)`로 제거할 수 있다.
- `EncodeSockaddrInet`의 `GetAddressBytes()` 결과 배열은 `TryWriteBytes(Span<byte>, out int)`로 제거할 수 있다.
- `IPAddress`/`IPEndPoint` 객체는 public `EndPoint` 계약과 Broker UDP runtime target 모델 때문에 이 국소 단위에서 제거하지 않는다.
- remote endpoint cache, value-type endpoint 계약, receive registration reuse와 IPv6는 D242에 포함하지 않는다.
- current TODO는 환경에 안정적인 allocation assertion Red와 두 helper의 Span 기반 최소 Green을 확정하는 것이다.

## 2026-07-15 (Codex - D241 transport registration과 pump 시작 원자성 보강)

### 검토 finding

- 최초 D241 구현은 Stop 뒤 `Register*`를 거부하지만 registration 성공과 pump 시작은 별도 문장이었다.
- Stop이 그 사이에 resource와 io_uring send-task 목록을 snapshot하면 Stop 완료 뒤 pump가 생성될 수 있었다.
- direct transport consumer의 종료 뒤 background I/O 금지 목표를 완결하려면 등록뿐 아니라 pump 생성·추적도 같은 종료 경계에 있어야 했다.

### Red/Green

- SAEA/RIO/io_uring에서 pump-start delegate를 포함하는 원자적 registration seam을 요구한 test는
  각각 `Assert.NotNull` expected non-null/actual null로 실패했다.
- connection과 UDP endpoint가 공유하는 `RegisterAndStartPumps` helper를 세 backend에 두고 기존 transport lock 안에서
  stopped 검사, 목록 등록, receive/send pump 생성과 io_uring send-task 추적을 완료했다.
- pump 시작 실패 시 목록 등록을 되돌리고 기존 caller catch/finally가 local owner를 정리하도록 유지했다.
- 기존 io_uring UDP reflection helper는 새 내부 인자를 전달하도록 갱신했다.

### 검증

- 신규 registration 경합 tests: SAEA/RIO/io_uring 3/3.
- backend projects: SAEA 45/45, RIO 58/58, io_uring 90/90.
- solution: 528/528, Release build 경고 0/오류 0.
- SAEA TCP load: 3000/3000, 99.8 Hz, p50 163.4 us, p99 833.8 us, HWM 1.
- SAEA TCP open-loop: 3000/3000, 99.9 Hz, p50 196.6 us, p99 892.6 us, HWM 4.
- SAEA UDP load: 3000/3000, 99.9 Hz, p50 150.6 us, p99 907.0 us, UDP HWM 1.
- SAEA UDP open-loop: 3000/3000, 99.9 Hz, p50 151.6 us, p99 839.4 us, UDP HWM 4.
- RIO TCP load: 3000/3000, 99.8 Hz, p50 185.4 us, p99 2070.5 us, HWM 1.
- RIO TCP open-loop: 3000/3000, 99.9 Hz, p50 190.8 us, p99 1445.7 us, HWM 2.
- RIO UDP load: 3000/3000, 99.8 Hz, p50 168.8 us, p99 1779.6 us, UDP HWM 1.
- RIO UDP open-loop: 3000/3000, 99.8 Hz, p50 173.5 us, p99 1384.8 us, UDP HWM 2.
- 여덟 target run 모두 drop 0, payload error 0, pool rented 0이다.
- io_uring native 실기기 재검증은 이번 로컬 단위에 포함하지 않았고 기존 Linux gate evidence를 유지한다.

### 다음 진입점

- 이번 review finding 보강 결과를 사용자 검토로 닫는다.
- push는 사용자가 별도 수행하며 다음 finding을 같은 commit에 섞지 않는다.

## 2026-07-15 (Codex - transport lifecycle 경합 hardening 구현)

### Red

- TCP listen과 UDP bind를 fake signal로 차단한 상태에서 Stop이 release 전에 완료되는지 검증했다.
  두 테스트 모두 `Assert.False`의 expected false/actual true로 실패해 server resource 게시 race를 재현했다.
- RIO/io_uring을 먼저 Stop한 뒤 private `RegisterConnection`을 호출했다.
  두 테스트 모두 expected `TargetInvocationException`이 발생하지 않아 종료 후 등록 허용을 재현했다.
- transport Stop이 예외를 내는 Dispose 뒤 Start를 다시 호출했다.
  expected `ObjectDisposedException`이 발생하지 않아 Dispose 종료 표식이 Stop 예외로 건너뛰는 경계를 재현했다.

### Green과 구조

- `BrokerServer`의 TCP/UDP Start와 Stop 전체를 control-path `SemaphoreSlim`으로 직렬화했다.
- 기존 start/stop 본문은 private core로 이동하고 `_gate` 기반 짧은 상태 전환과 cleanup 순서를 유지했다.
- Dispose는 `_disposed`를 기존 lock 안에서 먼저 게시한 뒤 직렬화된 Stop을 호출한다.
- RIO/io_uring의 listener, connection, UDP endpoint 등록은 각 transport lock 안에서 `_stopped`를 검사한다.
- RIO completion port snapshot/null 전환도 Stop lock 안으로 옮기고 UDP 등록 실패 시 endpoint native owner를 dispose한다.
- public API, receive/send/publish hot path, default backend와 transport restart 의미는 변경하지 않았다.

### 검증

- focused/project: Server 40/40, RIO 57/57, io_uring 89/89, SAEA Transport 44/44.
- solution: 525/525 통과.
- Release build: 경고 0, 오류 0.
- SAEA TCP load: 3000/3000, 99.6 Hz, p50 179.9 us, p99 907.5 us, HWM 1.
- SAEA TCP open-loop: 3000/3000, 99.9 Hz, p50 182.6 us, p99 861.5 us, HWM 2.
- SAEA UDP load: 3000/3000, 99.9 Hz, p50 157.3 us, p99 1080.5 us, UDP HWM 1.
- SAEA UDP open-loop: 3000/3000, 99.9 Hz, p50 151.3 us, p99 978.1 us, UDP HWM 6.
- 네 target run 모두 drop 0, payload error 0, pool rented 0이다.

### 다음 진입점

- D241 구현 결과의 사용자 review stop을 닫는다.
- push는 사용자가 수행하며 hot-path allocation과 current-head io_uring 성능 evidence는 별도 단위로 유지한다.

## 2026-07-15 (Codex - transport lifecycle 경합 hardening 구현 계획)

### 설계 승인과 실행 경계

- 사용자 진행 승인으로 D241 written spec 검토를 닫고 설계 상태를 Accepted로 전환했다.
- production code와 tests는 아직 수정하지 않고 별도 implementation-plan review stop을 유지했다.
- 구현 단위는 `BrokerServer`, RIO, io_uring lifecycle race만 포함하며 hot-path allocation, 성능 evidence와
  `BipBuffer` 검증은 범위 밖으로 유지했다.

### 계획 결과

- `docs/superpowers/plans/2026-07-15-transport-lifecycle-race-hardening.md`를 추가했다.
- fake listen/bind 차단을 사용하는 server Red 2개와 reflection 기반 native registration Red 2개를 exact code와 명령으로 고정했다.
- server Green은 기존 본문을 보존하는 lifecycle gate wrapper/core 분리로 제한했다.
- Dispose가 Stop 뒤 `_disposed`를 기록하면 Stop gate 반납 직후 Start가 진입할 수 있어, 기존 lock 안에서 종료 표식을 먼저 게시하도록 보완했다.
- native Green은 `_stopped` 전용 locked guard, RIO completion port snapshot/null 잠금 전환과 RIO UDP 실패 cleanup으로 제한했다.
- 계층별 focused/project tests, SAEA 대조, solution Release build/tests와 Windows TCP/UDP 4096B x 100Hz
  load/open-loop를 검증 순서로 명시했다.
- 전체 gate 실패 시 depth, retry, timeout 또는 새 abstraction으로 범위를 넓히지 않고 설계 재검토로 돌아가도록 했다.

### 다음 진입점

- implementation plan 사용자 검토를 닫은 뒤 Red 4개를 production 변경 전에 각각 확인한다.
- 설계 commit `f814cc1`과 이번 plan/state commit의 원격 반영은 사용자가 별도로 수행한다.

## 2026-07-15 (Codex - transport lifecycle 경합 hardening 설계)

### 확인된 문제

- `BrokerServer`는 start flag를 먼저 설정하고 비동기 listener/endpoint 생성 뒤 resource를 게시한다.
- 이 사이 Stop이 실행되면 아직 게시되지 않은 resource를 놓치고, Start가 종료 뒤 resource를 다시 게시할 수 있다.
- RIO/io_uring은 public method 진입 시 실행 상태를 확인하지만 `RegisterListener`, `RegisterConnection`,
  `RegisterUdpEndpoint` 안에서 `_stopped`를 다시 확인하지 않는다.
- SAEA는 register lock 안에서 실행 상태를 재검사하므로 native backend와 차이가 있었다.

### 설계 결과

- server lifecycle operation 전체를 control-path async gate 하나로 직렬화한다.
- RIO/io_uring 모든 `Register*`는 transport lock 안에서 Stop 이후 등록을 거부한다.
- RIO completion port snapshot/null 전환도 transport lock 안으로 옮긴다.
- TCP/UDP Start-Stop race와 native 종료 후 등록을 deterministic Red 4개로 고정한다.
- public API, hot path, backend 선택, restart 의미는 바꾸지 않는다.
- production code와 tests는 written-spec 검토 뒤 별도 구현 단위에서 변경한다.

### 상태 정합성

- `master`와 `origin/master`가 `216a35e89202c40a9ff2adf7404af8345cc8d92b`에서 ahead/behind 0/0임을 확인했다.
- `PLAN.md`의 오래된 Phase 1 snapshot과 `TODOS.md`의 완료된 push blocker를 현재 상태에 맞게 정리했다.
- 이번 written spec/state commit은 사용자 검토 뒤 원격에 반영하도록 `P1_SOON`으로 남겼다.

## 2026-07-14 (Codex - current-head io_uring 원격 gate review stop 종료)

### 상태 재평가

- 사용자의 다음 진행 승인으로 run `29305055740`의 build/TRX/native evidence 검토를 완료 처리했다.
- 현재 목표와 열린 요구 기준으로 즉시 실행 가능한 production code 작업은 남지 않았다.
- RIO IPv6는 default promotion 요구, server diagnostics는 실제 consumer, workflow allow-list는 실제 scope regression이
  각각 없으므로 deferred 우선순위를 유지한다.
- 원격 gate 기록과 review stop 종료 문서 커밋은 로컬에 완료했다.
- direct `git push`는 현재 실행 정책에서 차단돼 사용자가 `master`를 push하는 단계를 `P1_SOON`으로 남겼다.
- production code와 tests는 변경하지 않았다.

## 2026-07-14 (Codex - current-head io_uring 원격 Linux gate 갱신)

### 원격 동기화와 실행

- local HEAD와 `origin/master`가 `d63f3ba8147df4534268f851379dc05a3cb59427`에서 일치함을 확인했다.
- explicit `io_uring Linux Contract` workflow를 `master`에 dispatch했다.
- run `29305055740`, job `86996640536`은 2026-07-14T04:05:18Z에 성공으로 완료됐다.
- io_uring tests와 sample broker의 project-scoped restore/build를 포함한 모든 step이 성공했다.

### Artifact 검증

- artifact `iouring-linux-contract-2026-07-14-github-29305055740-1`을 임시 경로에 내려받았다.
- `iouring-tests.trx`는 total/executed/passed 88, failed/error/timeout 0이었다.
- capability evidence는 `io_uring capability status: Available`이었다.
- production registered payload 경로는 `registered payload fixed send path: hit`를 기록했다.
- Blame data collector warning은 모든 테스트가 끝나 sequence file을 만들지 않았다는 정상 종료 메시지였다.

### 결과

- D236의 explicit sample Linux contract가 RIO UDP depth 4 변경을 포함한 current HEAD에서도 유지됨을 확인했다.
- 이 결과를 end-to-end zero-copy, default transport 승격 또는 성능 우위 근거로 확대하지 않는다.
- production code와 tests는 변경하지 않았고 새 구현 단위를 자동으로 열지 않는다.

## 2026-07-14 (Codex - RIO UDP depth 4 구현 review stop 종료)

### 상태 재평가

- 사용자의 다음 진행 승인으로 depth 4 구현 결과 review stop을 닫았다.
- 현재 목표와 열린 요구를 대조했으며 즉시 실행 가능한 로컬 코드 작업은 남지 않았다.
- RIO IPv6는 default promotion 요구, server diagnostics는 실제 consumer, workflow allow-list는 실제 scope regression이
  각각 없으므로 deferred 우선순위를 유지한다.

### 다음 진입점

- 사용자가 push 가능해지면 현재 local commit을 원격에 반영한다.
- 이후 explicit io_uring Linux workflow의 project build와 TRX failure counter, registered payload native evidence를 갱신한다.
- production code와 tests는 변경하지 않았다.

## 2026-07-14 (Codex - RIO UDP fixed depth 4 반복 안정성 보강)

### Red와 최소 Green

- RIO UDP preflight는 smoke 8/8, drop/payload error/pool rented 0이었고 기존 focused tests는 18/18이었다.
- blocked-handler burst test는 expected 5 / actual 3으로 실패했다.
- close-owner test는 단독 실행에서 expected 5 / actual 3으로 실패했다. 두 Red 동시 실행에서는 첫 실패 cleanup 영향으로
  두 번째 actual이 0이어서 독립 실행으로 depth 2 한계를 재확인했다.
- production 변경은 `RioUdpEndpoint.ReceiveWindowSize` 2→4 한 줄뿐이며 `RioTransport.cs`는 변경하지 않았다.

### 회귀 검증

- 강화 test: 2/2 통과.
- `RioTransportUdpTests`: 17/17 통과.
- `Hps.Transport.Rio.Tests`: 56/56 통과.
- solution Release build: 경고 0/오류 0.
- solution tests: 520/520 통과.

### 반복 gate

- RIO UDP load/open-loop 각 3회가 모두 sent/received 3000/3000, drop 0, payload error 0, pool rented 0이었다.
- load actual rate 99.8~100.0 Hz, p50 175.0~180.4 us, p99 1039.0~1424.9 us, HWM 1이었다.
- open-loop actual rate 99.9~100.0 Hz, p50 176.2~192.4 us, p99 1454.2~2131.8 us, HWM 2~4였다.
- summary hard failure는 0이고 p99 warning 2개는 report-only였다.
- raw report 6개는 고유 임시 경로에서 구조적으로 검증했고 repository baseline으로 채택하지 않았다.

### 결과

- D240 fixed depth 4 가설을 수락했다.
- RIO explicit opt-in, SAEA library default, IPv4-only, latency report-only 경계는 유지한다.
- 사용자 구현 결과 review 전에는 추가 구현이나 push를 시작하지 않는다.

## 2026-07-13 (Codex - RIO UDP depth 4 hardening 구현 계획)

### 계획 경계

- approved spec을 `docs/superpowers/plans/2026-07-13-rio-udp-repeat-stability-hardening.md`로 구체화했다.
- production 변경은 `RioUdpEndpoint.ReceiveWindowSize` 2→4 한 줄을 우선한다.
- test 변경은 fixed-four burst Red, close owner Red, 중복 one-additional test 제거로 제한한다.
- `RioTransport.cs`, Broker, Protocol, benchmark schema/workflow는 evidence 없이 수정하지 않는다.

### 실행과 실패 정책

- RIO availability를 smoke로 확인한 뒤 두 assertion Red를 먼저 재현한다.
- focused/full RIO와 solution build/test 이후 UDP baseline suite를 runs 3으로 실행한다.
- load/open-loop 각 3회 모두 3000/3000과 hard failure 0일 때만 production fix를 커밋한다.
- 실패하면 task-owned code/test를 복원하고 실제 failure metrics와 diagnostics next step만 문서 커밋한다.
- 성공/실패 어느 쪽도 중간 commit, depth 8 확대, push를 수행하지 않는다.
...
## 2026-07-11 (Codex - RIO UDP depth 4 반복 안정성 hardening 설계)

### 결정

- D240의 다음 hardening을 internal fixed receive depth 4 검증으로 구체화했다.
- 기존 `RioUdpReceiveSlot`, request-context mapping, handler dispatch 직렬성을 그대로 재사용한다.
- public 설정, adaptive depth, 새 queue/worker/diagnostics API는 추가하지 않는다.

### 소유권과 TDD

- 정상 blocked-handler 상태의 receive payload owner는 handler current 1개와 posted slot 4개, 합계 5개다.
- close 또는 handler 예외 drain 뒤 `ReceivePool.RentedCount`는 0이어야 한다.
- 기존 depth 2 burst test를 추가 datagram 4개 보존 Red로 강화하고 close test도 expected 5 Red로 강화한다.
- 두 Red가 assertion failure로 확인되기 전에는 `ReceiveWindowSize`를 변경하지 않는다.
- Green은 내부 상수 2→4를 우선하며 generic owner 결함이 드러날 때만 production 범위를 넓힌다.

### 수락 및 실패 조건

- Release RIO UDP baseline suite를 `--runs 3`으로 실행한다.
- load/open-loop 각 3회 모두 3000/3000, drop/payload error/pool rented 0이어야 구현을 fix로 수락한다.
- latency와 HWM은 report-only로 기록한다.
- 한 번이라도 delivery hard fail이면 depth 8로 확대하지 않고 변경을 되돌린 뒤 누락 위치 diagnostics 설계로 돌아간다.

### 문서와 범위

- written spec: `docs/superpowers/specs/2026-07-11-rio-udp-repeat-stability-hardening-design.md`.
- 과거 D117/D118 설계와 evidence는 역사 기록으로 유지하고 새 spec은 D240 이후 보강만 다룬다.
- receive registration reuse, IPv6, default promotion, io_uring 원격 gate는 이번 단위와 섞지 않는다.

## 2026-07-11 (Codex - RIO TCP/UDP 3회 반복 안정성 gate)

### 범위

- 현재 checkout의 Release `Hps.Benchmarks`로 RIO TCP와 UDP baseline suite를 각각 `--runs 3`으로 실행했다.
- profile은 payload 4096 bytes, target 100 Hz, duration 30초이며 load/open-loop 각각 3개 report다.
- TCP와 UDP는 순차 실행했고 raw report와 summary는 고유 임시 디렉터리에만 보존했다.

### 결과

- RIO TCP load 3회는 actual 99.6~100.0 Hz, p99 median/max 1141.5/1409.6 us, HWM max 1이다.
- RIO TCP open-loop 3회는 actual 99.9~100.0 Hz, p99 median/max 1301.9/1388.5 us, HWM max 4이다.
- TCP 6개 report는 모두 sent/received 3000/3000, drop 0, payload error 0, pool rented 0으로 hard pass했다.
- RIO UDP load 3회는 모두 3000/3000, p99 median/max 1169.0/2776.2 us, HWM max 1로 hard pass했다.
- RIO UDP open-loop 3회는 sent 3000에 received 2996/2997/2999, HWM max 2로 모두 hard fail했다.
- open-loop actual 85.6~85.7 Hz는 30초 송신 뒤 5초 receive timeout이 elapsed에 포함된 결과다.
- UDP summary는 source report 6, hard failure 3, warning 6이다. latency warning은 report-only지만 delivery mismatch는 hard failure다.

### 조사

- 같은 binary와 환경에서 SAEA UDP open-loop를 대조 실행해 3000/3000, 99.9 Hz, p99 673.3 us, HWM 3으로 통과했다.
- RIO TCP와 UDP load가 통과하고 SAEA UDP open-loop도 통과해 benchmark 공통 하네스나 시스템 전체 부하 문제로 보기 어렵다.
- RIO UDP send queue HWM은 2이고 transport drop은 0이므로 bounded send queue overflow 증거는 없다.
- RIO UDP focused tests 18/18은 통과했다. 현재 blocked-handler depth 2 테스트는 짧은 bounded burst만 검증하며
  30초 open-loop에서 발생하는 scheduling 변동과 작은 delivery gap을 포착하지 못한다.
- D118의 3000/3000 한 번은 당시 구현 gate의 유효한 역사적 증거지만 반복 안정성을 확정하기에는 부족하다.

### 검증 및 다음

- Benchmark Release build: 경고 0, 오류 0.
- RIO UDP focused tests: 18/18 통과.
- production code와 tests는 변경하지 않았다.
- 다음 구현 전 단위는 bounded receive window depth 4 후보, blocked-handler Red, close/drain ownership,
  protocol별 반복 gate를 함께 다루는 최소 설계로 제한한다.
- raw artifact는 repository baseline으로 채택하지 않았고 default transport 의미도 변경하지 않았다.

## 2026-07-11 (Codex - UDP pending-send HWM summary 수정)

### 문제

- raw report와 `BaselineReport`는 TCP/UDP pending-send HWM을 모두 보존한다.
- `BaselineSummaryGenerator`의 kind min/max와 warning은 TCP HWM만 읽어 UDP summary를 0으로 만들었다.
- history/envelope는 summary의 legacy `tcp-hwm-*` field를 읽으므로 누락이 후속 artifact까지 전파됐다.

### Red

- UDP HWM 1/3만 가진 open-loop reports의 summary min/max test가 expected 1, actual 0으로 실패했다.
- UDP HWM 8 report의 기존 `open-loop-tcp-hwm-high` warning test가 empty collection으로 실패했다.

### Green

- `GetSendQueueHighWatermark`가 TCP/UDP HWM의 max를 반환한다.
- kind summary min/max와 per-run warning이 같은 helper를 사용한다.
- JSON `tcp-hwm-*`, `BaselineKindSummary.TcpHighWatermark*`, warning code/metric은 기존 artifact 호환성을 위해 유지했다.

### 검증

- focused UDP HWM tests: 2/2 통과.
- `Hps.Benchmarks.Tests`: 118/118 통과.
- solution build: 경고 0, 오류 0.
- solution tests: 실패 0, 통과 521.
- 기존 SAEA UDP raw report를 CLI로 재요약해 load HWM min/max 1/1,
  open-loop HWM min/max 3/3, hard-passed true를 확인했다.
- 독립 리뷰는 Critical/Important/Minor finding이 없었다.
- TCP와 UDP HWM이 동시에 양수인 synthetic mixed report 직접 test는 없지만 production helper가 명시적 `Math.Max`이고
  실제 runner는 protocol 하나만 활성화하므로 residual risk는 낮다.

## 2026-07-10 (Codex - 현재 checkout explicit RIO TCP/UDP gate)

### 범위

- Windows 현재 checkout에서 explicit RIO TCP/UDP smoke를 먼저 실행했다.
- 같은 Release binary로 4096 bytes x 100 Hz x 30초 load/open-loop를 protocol별 1회 순차 실행했다.
- raw report와 summary는 고유 임시 디렉터리에 두고 repository baseline에는 넣지 않았다.

### 결과

- TCP load: actual 99.8 Hz, p50 156.4 us, p99 874.1 us, TCP HWM 1.
- TCP open-loop: actual 100.0 Hz, p50 165.7 us, p99 1024.8 us, TCP HWM 2.
- UDP load: actual 99.9 Hz, p50 134.6 us, p99 818.5 us, UDP HWM 1.
- UDP open-loop: actual 100.0 Hz, p50 142.5 us, p99 1229.7 us, UDP HWM 2.
- TCP/UDP smoke와 baseline 모두 hard-passed true, warning-count 0,
  sent/received 일치, drop/payload-error/pool-rented 0이다.
- 같은 runner/profile RIO reference가 없어 envelope 비교나 SAEA 대비 성능 우위 주장은 하지 않는다.

### Build 환경 복구

- 첫 Release build는 restricted network의 vulnerability index 조회 실패로 `NU1900`을 기록했다.
- restore가 `project.assets.json` package root를 `C:\Users\CodexSandboxOffline\.nuget\packages`로 바꾼 뒤
  실제 재컴파일이 missing analyzer/package path로 실패함을 재현했다.
- 사용자 cache에 패키지가 존재함을 확인하고 `NUGET_PACKAGES=C:\Users\ADMIN\.nuget\packages`,
  `NuGetAudit=false`로 restore해 assets root를 복구했다.
- 복구 후 Release build 경고 0/오류 0과 재빌드 binary의 TCP/UDP RIO smoke pass를 확인했다.
- project/source/config 파일은 변경하지 않았다.

### 발견한 후속 결함

- raw UDP report의 pending-send HWM은 SAEA 1/3, RIO 1/2지만 summary는 TCP HWM만 집계해 0/0을 출력한다.
- 현재 gate는 raw UDP field를 직접 읽어 바로잡았다.
- summary/history/envelope 호환성 수정은 별도 `P1_SOON` TDD 단위로 남겼다.

## 2026-07-10 (Codex - 현재 checkout Release SAEA TCP/UDP gate)

### 범위

- 현재 checkout의 `Hps.Benchmarks`를 Release로 빌드했다.
- repository를 더럽히지 않는 임시 디렉터리에서 TCP/UDP `--baseline-suite --runs 1`을 순차 실행했다.
- profile은 payload 4096 bytes, target 100 Hz, duration 30초이며 load/open-loop 각각 3000메시지다.

### 결과

- build: 경고 0, 오류 0.
- TCP load: actual 99.9 Hz, p50 141.9 us, p99 455.0 us, HWM 1.
- TCP open-loop: actual 100.0 Hz, p50 150.7 us, p99 675.1 us, HWM 2.
- UDP load: actual 99.8 Hz, p50 128.7 us, p99 734.8 us, UDP HWM 1.
- UDP open-loop: actual 100.0 Hz, p50 152.2 us, p99 1023.6 us, UDP HWM 3.
- TCP/UDP 모두 hard-passed true, warning-count 0, drop/payload-error/pool-rented 0이다.

### Reference 비교

- 첫 TCP run은 `runner-id=local-unspecified`이라 `local-win-x64-01` reference와 key mismatch가 났다.
- raw report를 수정하지 않고 `HPS_BENCHMARK_RUNNER_ID=local-win-x64-01`로 TCP를 별도 재측정했다.
- 재측정 summary는 reference summary 9개와 envelope-compatible true, envelope-signal-count 0이다.
- UDP는 같은 runner/profile repository reference가 없어 hard gate와 latency 관측만 기록했다.
- 단일 checkout gate artifact는 임시 경로에만 두고 repository baseline으로 자동 채택하지 않았다.

### 다음

- 현재 단위는 SAEA checkout gate review stop에서 멈춘다.
- 다음 backend gate는 target OS가 Windows면 explicit RIO, Linux면 explicit io_uring 중 하나만 선택한다.

## 2026-07-10 (Codex - D239 Benchmark 실행/reporting 경계 설계)

### 조사

- `tests/Hps.Benchmarks`는 project file 포함 48개 파일이고 reporting 계열은 32개다.
- Hps runtime 또는 BenchmarkDotNet을 직접 참조하는 파일은 5개다.
- `Hps.Benchmarks.Tests` 16개 중 reporting/mixed CLI 관련 파일은 12개다.
- Windows/Linux workflow는 reporting command를 합계 9회 호출한다.
- reporting 구현을 직접 사용하는 `src/` 또는 `samples/` 소비자는 없다.
- 실행 영역의 raw JSON writer와 reporting 영역의 file reader 사이에 이미 자연스러운 파일 경계가 있다.

### 결정

- 별도 reporting library는 tool 책임은 그대로인데 project 연결만 늘려 제외했다.
- 별도 reporting executable은 책임은 명확하지만 parser, tests, workflow, 현재 문서를 함께 이관해야 해 현재 가치보다 비용이 크다.
- 하나의 executable/test project를 유지하고 raw report JSON을 실행/reporting 논리 경계로 고정한다.
- 독립 소비자, 의존성 충돌, 반복 cross-domain 회귀, 측정 가능한 workflow 비용이 생길 때만 물리 분리를 재평가한다.

### 검증 및 다음

- file/LOC, type reference, project reference, workflow command, 현재 문서 소비를 `rg`와 source read로 대조했다.
- spec placeholder와 범위/결정 모순을 확인했고 구현 계획이 필요 없는 no-change decision으로 정리했다.
- production code, tests, workflow와 기존 `.claude/review` 파일은 변경하지 않았다.
- 다음 실행 지점은 D239 written design 사용자 검토다.

## 2026-07-10 (Codex - D238 subscription readiness seam 구현)

### Red

- public method shape test가 `Assert.NotNull() Failure`로 실패해 API 부재를 확인했다.
- async 예외 검증의 첫 작성은 xUnit2014 analyzer 오류였으므로 유효한 Red로 인정하지 않고 `ThrowsAsync`로 수정했다.
- skeleton을 대상으로 timeout, cancellation, minimum, timeout 범위, null/empty topic의 assertion failure 6건을 확인했다.
- 독립 리뷰에서 마지막 poll이 deadline 뒤 count를 성공으로 수락할 수 있음을 발견했다.
- 1ms timeout 뒤 5ms에 등록하는 회귀 test가 `No exception was thrown`으로 실패해 finding을 재현했다.

### Green 및 이관

- `BrokerServer.WaitForSubscriberCountAsync`에 즉시 완료 fast path와 10ms polling, timeout/cancellation을 구현했다.
- API 의미는 특정 endpoint나 이후 delivery를 보장하지 않는 transient aggregate minimum count 관측으로 제한했다.
- Dashboard와 Benchmark TCP/UDP 네 호출부에서 private `_subscriptions` reflection과 중복 polling helper를 제거했다.
- Server loopback tests의 count wait를 public API로 옮기고 target identity 목적의 기존 white-box helper는 유지했다.
- `Hps.Benchmarks`에서 더 이상 필요 없는 `Hps.Broker` using과 project reference를 제거했다.
- 마지막 poll은 남은 시간만 지연하고 깨어난 뒤 deadline을 count보다 먼저 판정하도록 수정했다.
- public XML에 입력/timeout/cancellation 예외 계약을 명시하고 대기 중 취소와 음수 timeout test를 추가했다.

### 검증

- focused API tests 9/9, Server 37/37, Dashboard 13/13, Benchmark 116/116 통과.
- solution build 경고 0/오류 0, solution tests 실패 0/통과 519.
- 네 cross-module 호출부의 `_subscriptions`/`BindingFlags`/`SubscriptionTable` match 0,
  Benchmark의 `Hps.Broker`/`SubscriptionTable` 직접 참조 0을 확인했다.
- wire ACK, event/snapshot, endpoint별 readiness, publish hot path는 변경하지 않았다.
- 다음 실행 지점은 D238 구현 review stop이다.

## 2026-07-10 (Codex - D238 subscription readiness seam 설계)

### 확인 내용

- Dashboard TCP/UDP smoke와 Benchmark TCP/UDP runner 네 곳이 private `_subscriptions`를 reflection으로 읽는다.
- 모두 in-process isolated `BrokerServer`를 생성하며 count가 1이 될 때까지 같은 10ms polling을 수행한다.
- TCP/UDP handler는 SUBSCRIBE 처리 중 `SubscriptionTable`을 동기적으로 갱신한다.
- `Hps.Benchmarks`의 `Hps.Broker` 직접 참조는 reflection으로 얻은 `SubscriptionTable` 타입에만 사용된다.
- Server tests의 target identity assertion은 count wait와 다른 white-box 목적이다.

### 결정

- `BrokerServer.WaitForSubscriberCountAsync` public method 하나로 cross-module reflection과 polling을 수렴한다.
- wire ACK는 UDP reliability 범위를 열고, PUBLISH probe는 잔여 payload/측정 오염 위험이 있어 제외한다.
- event/snapshot, raw getter, 새 project, friend assembly는 만들지 않는다.
- 상세 설계: `docs/superpowers/specs/2026-07-10-subscription-readiness-seam-design.md`.

### 검증 및 다음

- 네 호출처, handler/table mutation, D068, project references를 `rg`와 source read로 대조했다.
- production code/tests는 변경하지 않았다.
- 다음 실행 지점은 written design 사용자 검토다.

## 2026-07-10 (Codex - Sample Broker selector surface 단순화)

### 작업 단위

- executable sample에만 있는 transport selector를 public library처럼 보호하던 compatibility overload를 제거했다.
- public mode 동작과 Program composition은 유지하고 selector surface와 테스트 연결만 단순화했다.

### Red

- `Selector_WhenInspected_ExposesSingleFullSelectEntryPoint`를 먼저 추가했다.
- focused 실행은 `Assert.Single() Failure: The collection contained 3 items`로 실패했다.
- 실패 collection은 4/5/7-argument public `Select` 세 개를 직접 보여 주었다.

### Green 및 Refactor

- `SampleTransportSelector`에서 사용되지 않는 4/5-argument overload를 제거했다.
- legacy overload 전용 `GetUnsupportedIoUringStatus`, `ThrowIoUringFactoryNotConfigured` helper를 제거했다.
- legacy overload test를 제거하고 기존 `saea`, `rio`, `auto`, `iouring` 정책 tests가
  실제 7-argument production entry를 직접 호출하게 했다.
- 구조 테스트는 파라미터 개수를 고정하지 않고 public `Select`가 하나라는 계약만 검증하도록 정리했다.

### 검증

- selector focused tests: 13/13 통과.
- Sample Broker tests: 25/25 통과.
...
- 다음 실행 지점은 네 곳의 private `_subscriptions` readiness reflection을 하나의 계약으로 수렴할 방향을 확정하는 것이다.

### 독립 리뷰

- Critical finding은 없었다.
- D233/D234와 기존 설계서가 4/5-argument overload 유지 결정을 담고 있어 superseded 표기를 추가했다.
- `SampleBrokerServerProjectContractTests`의 오래된 “기존 호환 오버로드” 주석을 단일 selector entry 표현으로 갱신했다.
