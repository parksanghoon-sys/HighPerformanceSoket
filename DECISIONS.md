...
- 첫 explicit runner baseline 인 `local-win-x64-01/2026-06-24`는 3-session reference 를 갖췄다.
  현재 explicit runner envelope 는 load p99 max 870.7 us, open-loop p99 max 1051.5 us 이며,
  hard gate, warning, comparison mismatch 는 없다. 다만 같은 runner 의 date root 가 아직 1개뿐이므로
  D082 warning-as-failure 또는 CI latency gate 승격 조건에는 산입하지 않는다.
- 다음 Phase 4 단위는 `local-win-x64-01/2026-06-25/session-01` explicit runner baseline 수집으로 둔다(D085).
