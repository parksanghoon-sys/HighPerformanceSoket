...
- [x] D233 sample broker explicit `--transport iouring` 설계를 작성했다.
  - 범위: sample transport mode/parser/selector/Program/project,
    Sample Broker tests, `iouring-linux-contract.yml`, workflow static contract.
  - 결정: explicit mode만 추가하고 unavailable은 exit code 1로 실패한다.
    기존 `auto`의 RIO/SAEA 의미와 library default는 유지한다.
  - TDD: 첫 Red는 문자열 parser assertion failure로 시작해 compile failure를 피하고,
    selector, project/Program, workflow contract 순서로 진행한다.
  - 원격 gate: sample broker Linux build step과 기존 io_uring native TRX evidence를 조합한다.
  - 산출물: `docs/superpowers/specs/2026-07-10-sample-broker-explicit-iouring-transport-design.md`.
  - 다음: 사용자 설계 검토 후 implementation plan 작성 여부를 결정한다.
...
- [x] D226 queue-scoped registered payload pool 설계를 작성했다.
  - 범위: D225 후속 범위 설계, `PinnedBlockMemoryPool`, `RefCountedBuffer`,
    `TcpFrameAssembler`, `BrokerPublisher`, `IoUringFixedSendBufferRegistry`, `IoUringTransport.SendInFlightAsync`.
  - 결과: production TCP payload `WRITE_FIXED` 연결 전 단계로 TCP-first registered payload block pool 을 설계했다.
    `RefCountedBuffer` 반환 owner abstraction, assembler source abstraction,
    io_uring internal registered payload pool, miss fallback, UDP 제외 범위를 정리했다.
  - 산출물: `docs/superpowers/specs/2026-07-09-iouring-registered-payload-pool-design.md`.
  - 다음: 사용자가 D226 설계를 검토한 뒤 구현 계획 작성 여부를 결정한다.
...
- [x] D200 Task 3 Linux native lease evidence 를 로컬 구현했다.
  - 범위: `tests/Hps.Transport.IoUring.Tests/IoUringFixedSendLeaseTests.cs`,
    `docs/superpowers/plans/2026-07-07-iouring-fixed-send-lease-owner.md`, root 상태 문서.
  - Red: `LinuxSocketPair_HelperExistsForLeaseNativeEvidence`가 helper type 부재로 `Assert.NotNull() Failure`를 냈다.
  ...
  - 결과: Linux capability available 환경에서 lease 가 소유한 registered buffer slice 를
    stream socket fd 로 `TrySubmitWriteFixed` 제출하고 `{20,30}` payload 를 읽도록 검증한다.
  - 검증: focused native evidence test 로컬 guard 통과,
    `Hps.Transport.IoUring.Tests` 69개 통과,
    `dotnet test HighPerformanceSocket.slnx -v minimal` 전체 통과.
  - 다음: push 이후 원격 `iouring-linux-contract.yml` artifact gate 를 검토한다.
...
- [x] D188 WPF/MVVM sample dashboard Task 5 UDP smoke service 를 구현했다.
  - 범위: `samples/Hps.Sample.Dashboard/Services/UdpSmokeTestService.cs`,
    `tests/Hps.Sample.Dashboard.Tests/UdpSmokeTestServiceTests.cs`,
    `docs/superpowers/plans/2026-07-06-wpf-sample-dashboard.md`, D189 상태 문서.
  - Red: `UdpSmokeTestService` type 부재 assertion failure 를 확인했다.
  - Green: 실제 SAEA UDP endpoint receive/send pump 와 Broker datagram handler fan-out 으로 UDP SUBSCRIBE/PUBLISH loopback 을 검증한다.
  - 검증: `dotnet test tests\Hps.Sample.Dashboard.Tests\Hps.Sample.Dashboard.Tests.csproj --filter UdpSmokeTestServiceTests -v minimal` 통과,
    `dotnet build samples\Hps.Sample.Dashboard\Hps.Sample.Dashboard.csproj -v minimal` 경고 0/오류 0.
  - 다음: Task 6 UI binding, run instructions, full verification 을 구현한다.
...
- [x] D170 reference 확장 이후 원격 `iouring-benchmark-artifacts.yml` artifact gate 를 검토했다.
  - 범위: GitHub Actions run `28569649366`,
    artifact `iouring-benchmark-artifacts-2026-07-02-github-28569649366-1`, D171 상태/결정 문서.
  - 결과: workflow success, TCP/UDP baseline/summary/history/envelope exit code 0.
  - evidence: TCP/UDP raw report count 는 각각 6이고, hard-passed true, drop/payload-error/pool-rented 0이다.
  - evidence: TCP envelope 는 reference-summary-count 5, compatible true, signal-count 0이다.
  - evidence: UDP envelope 는 reference-summary-count 8, compatible true, signal-count 0이다.
  - 의미: D170 reference history 가 원격 workflow artifact 에서 실제 envelope reference 로 사용됐다.

- [x] D169 기준으로 D168 raw report 를 protocol별 두 번째 date root session-02 reference 로 수동 채택했다.
  - 범위: `docs/benchmarks/baselines/runners/ci-linux-iouring-x64-01/tcp/2026-07-02/session-02`,
    `docs/benchmarks/baselines/runners/ci-linux-iouring-x64-01/udp/2026-07-02/session-02`,
    protocol별 history, `docs/benchmarks/baselines/index.md`, D170 상태/결정 문서.
  - 결과: TCP protocol root history 는 session-count 5, hard-passed true, warning-count 30,
    comparison-compatible true 상태다.
  - 결과: UDP protocol root history 는 session-count 8, hard-passed true, warning-count 16,
    comparison-compatible true 상태다.
  - 검증: 최신 session 기준 envelope smoke 는 TCP/UDP 모두 `envelope-compatible=true`,
    `envelope-signal-count=0`으로 통과했다.
  - 검증: baseline path absolute path scan 매칭 없음, `Hps.Benchmarks.Tests` 114개 통과, `git diff --check` 통과.

- [x] D168 원격 artifact gate 이후 io_uring 다음 작업 후보를 재평가했다.
  - 범위: `docs/superpowers/specs/2026-07-02-iouring-post-d168-reference-date-continuation-design.md`,
    D169 상태/결정 문서.
  - 결정: D168은 correctness/reliability failure 가 아니라 두 date root reference 기준 signal 0 passing evidence 이므로,
    fixed registration, zero-copy, latency gate, default promotion 대신 두 번째 date root session-02 수동 채택을 다음 단위로 둔다.

- [x] D167 reference date 확장 이후 원격 `iouring-benchmark-artifacts.yml` artifact gate 를 검토했다.
  - 범위: GitHub Actions run `28568500822`,
    artifact `iouring-benchmark-artifacts-2026-07-02-github-28568500822-1`, D168 상태/결정 문서.
  - 결과: workflow success, TCP/UDP baseline/summary/history/envelope exit code 0.
  - evidence: TCP/UDP raw report count 는 각각 6이고, hard-passed true, drop/payload-error/pool-rented 0이다.
  - evidence: TCP envelope 는 reference-summary-count 4, compatible true, signal-count 0이다.
  - evidence: UDP envelope 는 reference-summary-count 7, compatible true, signal-count 0이다.
  - 의미: D167 두 date root reference history 가 원격 workflow artifact 에서 실제 envelope reference 로 사용됐다.
