...
현재 권장 후보는 `BrokerServer + SaeaTransport` UDP broker socket loopback 통합 테스트 단위다.
이미 `BrokerServer.StartUdpAsync`가 `BrokerUdpDatagramHandler`를 등록하고 `BindUdpAsync` endpoint 수명을 관리하므로,
다음 단위는 실제 UDP socket subscriber/publisher 가 `SUBSCRIBE`/`PUBLISH` datagram 을 보내고 subscriber 가 payload 를 수신하는
end-to-end 경계를 검증한다. UDP stale remote idle expiry 는 뒤로 분리한다.

## 이번 단위의 검증 경로
- Red API: `dotnet test tests\Hps.Server.Tests\Hps.Server.Tests.csproj --no-restore --filter "FullyQualifiedName~BrokerServerContract_WhenInspected_ExposesMinimalUdpHostWiringApi"` 실패 1.
- Green API: 같은 계약 테스트 통과 1.
- Behavior Red: `dotnet test tests\Hps.Server.Tests\Hps.Server.Tests.csproj --no-restore --filter "FullyQualifiedName~StartUdpAsync_WhenCalled|FullyQualifiedName~StartTcpAsyncThenStartUdpAsync|FullyQualifiedName~StopAsync_WhenUdpStarted"` 실패 3.
- Green focused: 같은 behavior focused 테스트 통과 3.
- Server 전체: `dotnet test tests\Hps.Server.Tests\Hps.Server.Tests.csproj --no-restore` 통과 9.
