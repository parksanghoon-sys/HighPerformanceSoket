...
다음 cycle 은 TCP-first io_uring queue/pump implementation plan Task 4 TCP resource and listener wiring 을 TDD로 구현한다.

- 범위: `docs/superpowers/plans/2026-06-29-iouring-tcp-first-pump.md` Task 4,
  `IoUringConnectionListener`, `IoUringTcpConnectionResource`, `IoUringTransport` TCP boundary.
- 검증: reflection/behavior assertion Red, focused TCP boundary tests Green, io_uring test project,
...
- 현재 상태: io_uring source/test project, capability probe, opt-in transport root type 이 존재한다.
  ...
  operation registry/context, completion loop dispatch boundary 가 존재한다.
- 다음 산출물: TCP listen/accept resource boundary skeleton 과 non-Linux 명시적 unsupported 경계 보존.
...
- CI artifact 자동 채택, pull_request trigger, schedule trigger
- io_uring TCP receive/send pump 구현
- stable identity 인증/권한 검증, persistence, payload replay
