...
- mixed raw JSON은 `schema-version: 2`를 사용한다. 현재 `BaselineReportReader`는 version 1만 읽으므로 이 값이 legacy aggregate 격리 경계다.
...
- stream hard gate는 subscriber별 exact count/order/payload, actual rate 99% 이상, p99 5,000us 이하, p999 10,000us 이하다.
...
- `tests/Hps.Benchmarks/MixedWorkloadOptions.cs`: 고정 profile, 입력 검증, checked 계획 수 계산.
- `tests/Hps.Benchmarks/MixedWorkloadStreamResult.cs`: stream별 전달, rate, latency hard gate.
- `tests/Hps.Benchmarks/MixedWorkloadRunResult.cs`: 두 stream과 global diagnostics를 합친 최종 판정 및 console 출력.
- `tests/Hps.Benchmarks/MixedWorkloadReportWriter.cs`: legacy reader와 분리된 schema-version 2 JSON.
...
## Task 2: Mixed workload options와 checked 계획 수를 고정한다
...
- Produces properties: `DataRateHz`, `DurationSeconds`, `SubscriberCount`, `DataMessageCount`, `ControlMessageCount`, `DataDeliveryCount`, `ControlDeliveryCount`.
- Produces constants: payload 10,240/2,560B, default/min data rate 100Hz, control rate 100Hz, default duration 30초, default subscriber 1명, max frame 16,384B, rate ratio 0.99, p99/p999 5,000/10,000us.

- [ ] **Step 1: type 부재 assertion Red를 작성한다**
...
- [ ] **Step 2: 최소 type shell로 shape를 Green으로 만든다**
...
- [ ] **Step 3: 기본값, 사용자 입력과 overflow behavior Red를 추가한다**
...
Expected: count assertions and exception assertions fail against the shell, not compile.

- [ ] **Step 4: validation과 checked count를 최소 Green으로 구현한다**

생성자는 아래 순서로 검증하고 모든 계획 수를 constructor에서 한 번 계산해 readonly property에 저장한다.
...
try
{
    DataMessageCount = checked(dataRateHz * durationSeconds);
    ControlMessageCount = checked(ControlRateHz * durationSeconds);
...
`ControlRateHz`는 static constant와 이름이 충돌하므로 instance property를 만들지 않는다. test는 `MixedWorkloadOptions.ControlRateHz`로 읽는다.

- [ ] **Step 5: focused와 benchmark 전체를 Green으로 확인하고 commit한다**
...
Review stop: options/math만 보고하고 Task 3을 시작하지 않는다.

---

## Task 3: Stream/global hard gate와 schema-version 2 report를 만든다
...
- Produces: `MixedWorkloadStreamResult`의 `DeliveryPassed`, `RatePassed`, `LatencyBudgetPassed`, `Passed`, `ActualRateHz`, `P99LatencyGrowthRatio`.
- Produces: `MixedWorkloadRunResult`의 `Scenario`, `DurationSeconds`, `SubscriberCount`, `Data`, `Control`, `DroppedPendingSendCount`, `TcpPendingSendQueueHighWatermark`, `EndPendingSendCount`, `FallbackPoolRentedAfterStop`, `TimeoutCount`, `Identity`, `Passed`, `Print(TextWriter)`.
...
test helper `CreatePassingStream`은 data 기준으로 100 planned/sent, subscriber 2명, 200 deliveries, min/max 100, error 0, actual 100Hz, p99 4,000us, p999 9,000us를 만든다.

다음 test를 각각 둔다.

- passing stream은 `DeliveryPassed`, `RatePassed`, `LatencyBudgetPassed`, `Passed`가 모두 true다.
- sent가 planned보다 1 작으면 delivery false다.
- 한 subscriber의 count가 99라 `minimumReceivedPerSubscriber == 99`, `failedSubscriberCount == 1`이면 delivery false다.
- sequence error 또는 payload error가 1이면 delivery false다.
- actual rate가 target의 98.9%면 rate false, 99.0%면 true다.
- p99가 5,000.1us 또는 p999가 10,000.1us면 latency false다.
- data/control이 통과해도 drop, end pending, pool rented, timeout 중 하나가 1이면 run false다.
- 모든 global count가 0이면 run true다.

`publisherElapsedTicks`는 `Stopwatch.Frequency`를 사용해 test에서 정확히 1초로 만들고 `ActualRateHz == 100.0`을 단언한다.
...
public bool LatencyBudgetPassed
{
    get
    {
        return P99LatencyMicroseconds <= MixedWorkloadOptions.P99LatencyBudgetMicroseconds
            && P999LatencyMicroseconds <= MixedWorkloadOptions.P999LatencyBudgetMicroseconds;
    }
}
...
Assert.Equal(9000.0, root.GetProperty("streams")[0].GetProperty("p999-latency-us").GetDouble());
```

같은 directory를 `BaselineReportReader.ReadDirectory`로 읽었을 때 count 0도 단언한다. version 1을 쓰면 legacy reader가 mixed shape의 누락 key에서 예외를 내므로 Red가 된다.

- [ ] **Step 6: `Utf8JsonWriter`로 schema-version 2를 Green으로 구현한다**

top-level key 순서는 다음으로 고정한다.

```text
schema-version, result-name, passed, scenario,
...
duration-seconds, subscriber-count, max-frame-payload-bytes,
...
각 stream object는 constructor property 전체와 `actual-rate-hz`, 세 부분 gate, `passed`, `p99-latency-growth-ratio`, `publisher-elapsed-ms`를 쓴다. round 자릿수는 rate/latency 1자리, growth 2자리다.
...
6. subscriber별 reusable buffer에 length-prefixed `SUBSCRIBE <topic>`을 직접 쓰고 전송한다.
7. `WaitForSubscriberCountAsync`로 data/control 각각 1명을 확인한다.
8. 두 receive task와 두 publisher task를 시작하고 공통 `TaskCompletionSource<long>` start tick을 release한다.
9. 네 task를 완료한 뒤 endpoint pending 합이 0이 될 때까지 최대 10초 drain한다.
10. transport diagnostics와 endpoint snapshot을 캡처한다.
11. socket을 닫고 server를 Stop한 뒤 client block을 정확히 한 번 Return한다.
12. pool의 `RentedCount == 0`을 확인하고 result를 만든다.
...
remaining time이 2ms 이상이면 `Task.Delay(remainingMilliseconds - 1, token)`, 그 이하면 `Task.Yield()`로 target까지 접근한다. publisher는 첫 send 직전 tick과 마지막 `SendAllAsync` 완료 직후 tick을 기록한다. data와 control pacing state를 공유하지 않는다.

- [ ] **Step 6: subscriber 검증과 percentile 집계를 구현한다**

subscriber state는 계획 수 길이의 `long[] LatencyTicks`, `Received`, `SequenceErrors`, `PayloadErrors`, `TimedOut`만 가진다.
...
- p50/p99/p999는 유효 latency를 복사해 sort한 뒤 `ceil(count * percentile) - 1` index를 사용한다.
- first/second-half p99는 sequence 기준 앞/뒤 절반을 별도 배열로 집계한다.

Task 4에서는 subscriber state가 stream당 하나이므로 min/max는 해당 `Received`, failed count는 count/order/payload 중 하나라도 실패하면 1이다.
...
- [ ] **Step 3: stream aggregate를 subscriber별 exact gate로 만든다**

- received delivery count는 모든 subscriber `Received` 합이다.
- min/max는 subscriber state의 실제 `Received` 최소/최대다.
- failed subscriber count는 count 불일치, sequence error 또는 payload error가 있는 state 수다.
- sequence/payload error는 subscriber 전체 합이다.
- latency percentile은 모든 subscriber의 유효 sample을 계획 delivery 수 크기의 배열에 합쳐 계산한다.
- first/second-half p99는 각 subscriber의 sequence 절반을 각각 합쳐 계산한다.

aggregate 합만 일치해도 한 subscriber가 누락되면 `MinimumReceivedPerSubscriber`와 `FailedSubscriberCount`가 run을 실패시켜야 한다.
...
정수 parse 후 `new MixedWorkloadOptions(dataRate, duration, subscribers)`를 호출해 minimum과 checked count를 검증하고, `ArgumentOutOfRangeException`은 안정적인 usage message로 변환한다.
...
```text
Hps.Benchmarks --mixed-load-open-loop [--backend <saea|rio|iouring>] [--data-rate-hz <100+>] [--duration-seconds <1+>] [--subscribers <1+>] [--report <path>]
```
...
$json | Select-Object 'schema-version','result-name',passed,'subscriber-count','dropped-pending-send-count','end-pending-send-count','fallback-pool-rented-after-stop','timeout-count'
if ($exit -ne 0) { throw "mixed CLI smoke failed: $exit" }
```

Expected: exit 0, schema-version 2, result-name `mixed-load-open-loop`, global zero counters, streams 2개. latency가 threshold를 넘으면 commit하지 않고 raw report를 보존해 Task 4 계측/runner를 재검토한다.
...
각 JSON에서 두 stream `sent/planned`, `received/planned-delivery`, min/max, failed subscriber, sequence/payload error, actual rate, p99/p999와 global drop/HWM/pending/pool/timeout을 기록한다. 세 run 모두 `passed=true`여야 한다.
...
- mixed raw report 3개가 존재하고 모두 schema-version 2, passed true다.
...
- count가 overflow 전에 allocation/OOM으로 실패하면 parser가 runner까지 너무 큰 값을 통과시킨 것이다. 임의 max를 추가하지 말고 요구 상한을 설계에 명시한 새 review 단위로 올린다.
- mixed report를 같은 directory에서 legacy reader가 읽거나 예외를 내면 `schema-version: 2` 경계를 먼저 확인한다. baseline reader에 mixed 특별 분기를 추가하지 않는다.
...
- Spec coverage: options, CLI, protocol 거부, checked count, 두 stream topology, reusable pinned client buffer, shared start, independent pacing, per-subscriber exact gate, p50/p99/p999, drop/HWM/pending/leak/timeout, schema 격리, SAEA/RIO/io_uring/soak/fan-out evidence가 Task 2~8에 각각 연결되어 있다.
