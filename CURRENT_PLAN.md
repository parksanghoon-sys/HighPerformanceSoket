...
- 실제 command codec 연결과 assembler ↔ Transport recv loop 통합은 아직 후속 단위로 남아 있다.
- D013 기준으로 이번 기능 단위 완료 후 다음 구현은 사용자 리뷰 뒤 진행한다.

## 다음 단일 작업 단위
사용자 리뷰 대기.

리뷰 후 계속 진행 지시가 있으면 assembler ↔ Transport recv loop 통합(C1/C2/C3) 또는 command codec 중
현재 Phase 흐름과 리뷰 의견 기준으로 가장 작은 다음 단위를 다시 판단한다.
UDP receive backpressure 정책(Q1)은 fan-out/backpressure 결정과 맞물리므로 성급히 구현하지 않고 별도 설계 단위로 둔다.

## 이번 단위의 검증 경로
- `TcpFrameAssembler`의 기존 baseline 위에 0 length frame, 다중 frame consumed loop, maxPayload 정확 경계,
  결정적 chunk fragmentation fuzz 테스트를 추가한다.
- production code 변경 없이 기존 구현이 edge/fuzz 계약을 만족하는지 확인한다.
- `dotnet test tests\Hps.Protocol.Tests\Hps.Protocol.Tests.csproj --filter "FullyQualifiedName~TcpFrameAssemblerTests"`
...
- 결과: focused `TcpFrameAssemblerTests` 통과 7. Protocol 전체 통과 7.
  전체 `dotnet test HighPerformanceSocket.slnx`는 `Hps.Buffers.Tests` 통과 18 + `Hps.Transport.Tests` 통과 26 + `Hps.Protocol.Tests` 통과 7,
...
- assembler ↔ Transport recv loop 통합과 PayloadTooLarge 수신 시 connection close 정책
