  ...
  benchmark-artifacts:
    ...
    steps:
      ...
      - name: Run baseline suite
        shell: pwsh
        run: dotnet run --project tests/Hps.Benchmarks/Hps.Benchmarks.csproj -- --baseline-suite "$env:BENCH_SESSION_DIR" --runs 3

      - name: Write baseline summary
        shell: pwsh
        run: dotnet run --project tests/Hps.Benchmarks/Hps.Benchmarks.csproj --no-build -- --summarize-baseline "$env:BENCH_SESSION_DIR" --summary "$env:BENCH_SESSION_DIR/summary.json" --summary-md "$env:BENCH_SESSION_DIR/summary.md"

      - name: Write baseline history
        shell: pwsh
        run: dotnet run --project tests/Hps.Benchmarks/Hps.Benchmarks.csproj --no-build -- --summarize-baseline-history "$env:BENCH_DATE_ROOT" --history "$env:BENCH_DATE_ROOT/history.json" --history-md "$env:BENCH_DATE_ROOT/history.md"
